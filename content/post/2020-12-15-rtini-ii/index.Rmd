...
wasted quite a bit of time going in circles as a result.

# Some Triangles Are More Equal Than Others

There are eight different triangle orientations, split into two groups of four.
The groups alternate between approximation levels.  Here we show the
orientations for the 2nd and 3rd most granular levels of a 9 x 9
grid[^nine-nine].  The triangles are colored by orientation.  For reasons that
will become clearer later we call the group with diagonal hypotenuses
"Diagonal", and the one with vertical/horizontal ones "Axis":<span
id='sqr-vs-dia'></span>
...
(<span style='background-color: #8da0cb'>lavender</span>) and endpoints(hollow
<span style='color: #66c2a5'>green</span> ), and the corresponding child
...
The first step is to generate the base tile, which we will do with
...
coordinates[^linearized]<span id=linearized></span>:
...
```{r eval=FALSE}
init_offsets(i=1, j='diag', n=8, layers=log2(8))
```
...
-->
We annotate the original map with the "linearized" coordinates to better see the
relationship:
...
`(n + 1)`, where `n <- nrow(map) - 1`, shifts coordinates by one column.  So in:
...
times faster than the R transliteration of the RTIN!  With some additional
optimizations [I won't bore you with](#follies-in-optimization) ("R-vec-2") we
get to beat out JavaScript.  And something remarkable happens as we increase
grid sizes:
...
The R implementation ("R-vec-2" from the previous chart) beats both the C and JS
implementations by a factor of 2-3x.  At the lower sizes the overhead of the R
calls holds us back, but as the overall computations increase by `$N^2$`, the
number of R calls only increase by `$log(N)$`.  As the `$N^2$` term grows the
vectorized algorithm benefits from requiring fewer steps to compute the
midpoints.
...
width and height.  This allows us to render the 550 x 505 River Derwent DEM by
dropping just one row:
...
has no inconsistencies at the boundaries.
