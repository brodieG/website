...
in R.  Reading [Part I][1] will help to understand this post._
...
(<span id=lavender style='background-color: #8da0cb'>lavender</span>) and
endpoints(hollow <span style='color: #66c2a5'>green</span> ), and the
corresponding child hypotenuse midpoints (<span style='background-color:
#fc8d62'>orange</span>).  The parent/child relation is shown with the arrows:
...
points of the template tile in the same colors as [previously](#lavender):
...
<video 
  id=derwent-vid style='display: block; margin: 0 auto;' controls loop
...
<figcaption>River Derwent DEM, Low Polygon Count, Kaleidoscope Style.</figcaption>
</figure>
</div>
...
result of this we now have a base-R-only mesh reduction algorithm that works on
a much broader set of meshes than the original RTIN.
