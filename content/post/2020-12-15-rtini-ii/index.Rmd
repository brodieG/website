...
which in simplified[^simplified-offsets] form is:
...
(1,1), 2 to (1,2),  18 to (2,1), 289 to (17,17), etc.[^linearized].  So these
two index matrices match color-wise to:

```{r echo=FALSE}
err16 <- matrix(0, 17, 17)
 # note diamond/square is backwards here from what we use in viz-lib
tri.di <- extract_mesh2(err16, 1, 4)
tri.sq <- extract_mesh2(err16, 1, 3)
tris.a <- tris_to_df(list(tri.sq, tri.di), err16)
tris2.a <- transform(
  tris.a,
  alpha=ave(
    seq_along(id), factor(id), FUN=function(i) all(x[i] <= .25 & y[i] <= .25)
  )
)
lin.to.xy <- expand.grid(x=0:16, y=0:16)
lin.to.xy <- transform(lin.to.xy, i=(x + 1) + y * 16)
p2a <- ggplot(tris2.a) +
  geom_polygon(aes(x=x, y=y, group=id, fill=I(color)), alpha=.2) +
  geom_point(data=points, aes(x=x, y=y), size=1, shape=3, color='white') +
  # geom_segment(
  #   data=subset(p.arr, ptype=='child'),
  #   arrow=arrow(length=unit(.05, 'inches'), type='closed', angle=25),
  #   aes(x, y, xend=xend, yend=yend),
  ...
  geom_point(
    data=subset(p.all, ptype=='child'),
    aes(x, y, colour=I('#fc8d62')), size=2
  ) +
  geom_text(
    data=lin.to.xy, aes(x=y/16, y=x/16, label=i)
  ) +
  facet_wrap(~type) +
  coord_fixed(xlim=c(0, .25), y=c(0, .25)) +
  # coord_fixed() +
  thm.blnk
