...
And so began this quixotic crusade[^quixotic] to implement an "efficient-enough"
3D rendering pipeline using base R only.  I took notes along the way to share
with those who like me are interested, but unlike me have better time management
skills.  So, come along for a ride through 3D projections, meshes, perspective
adjustments, rasterization, and image processing, all in base R[^base-r-only].
...
the case, but the documentation is just ambiguous enough that I feel comfort
...
`plot_new` sets plot parameters (e.g. margins, aspect ratios).  `rescale`
rescales values to between zero and one.  Both of these  are [helper functions
defined in the appendix](#helper-functions).
...
exposition I put the code in the [shadow demo package][], but as far as I'm
...
While this looks rather wide for long wide, but I assure you it is long in
spirit.  It is just that we need it in 3 x n rather n x 3 format for the
upcoming matrix multiplication.  As applied to `volcano`:
...
green tile, we take the top left point of the same color from each of the
bounding boxes.  The same process is repeated for each tile by picking the
corresponding vertex from each panel.
...
theme when trying to write fast R code.
...
the idea is that the vector wrapping nature of the matrix allows us to easily
sandwich NA values between each of our tiles' data:
...
of](#meshes-with-pesp), but more importantly, "this" is not what we're after.
...
Everything we've done to this point has been assumed no perspective effect, so
that objects of the same size appear the same size irrespective of how far
from the observer they are.  This observer, you, has been looking at the object
down on a line parallel to the Z axis and passing through the midpoint of the X
...
In the following diagram you are looking down a line parallel to the X axis.
The Z axis now runs from left to right.  Observers 1, 2, and 3 are all still
looking at `volcano` down the Z axis:
...
And now we can visualize or bounding boxes:
...
This is the part of the process that feels dirtiest.  We will brute force
generate pixels to fill each of our bounding boxes.  Oddly this is a little
tricky, mostly because we wish to do this fully in internally vectorized code.
And it is particularly important that we do so because we are about to
dramatically increase the amount of data we are dealing with.  The plan of
attack is to compute all possible pixel "offset" permutations within each
bounding box, and then add back the min x and y coordinates to get the actual
pixel coordinates.

First we compute how dimensions of the bounding x/y min/max boxes in pixels:
...
allows us to implement much better shading.  Another factor is that polygon
rendering becomes expensive when the polygon counts increase.  Here we
illustrate with an elevation map that has fifty times as many points as
`volcano`:
...
With a little [`ffmpeg` magic](#videos) we have:
...
[^double-precision]: 3D rendering is typically done in single precision floating
  point, which would halve the memory footprint of the process.
