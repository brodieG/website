...
image: "/front-img/stereoscopic-square.png"
imagerect: "/front-img/stereoscopic-rect.png"
contenttype: article
imagemrgvt: "0%"
imagemrghz: "10%"
weight: 1
description: Step by step implementation of a 3D rendering pipeline, from
  projections, to meshes, to rasterization, using base R only.
categories: [r]
...
<img
src="/front-img/stereoscopic-square.png"
...
```
```{r height-map, fig.width=6, fig.height=4}
xv <- row(volcano)
yv <- col(volcano)
plot_new(xv, yv)
...
```{r height-map-rs, fig.width=6, fig.height=4}
library(shadow)
...
```{r tile-to-mesh, echo=FALSE, fig.width=8, fig.height=2}
...
```{r persp, echo=FALSE, fig.width=7, fig.height=2.33}
...
```{r persp-tryp, echo=FALSE, fig.width=9, fig.height=3.4}
...
```
```{r persp3d-full, echo=FALSE, eval=FALSE}
...
```{r rast-illust, echo=FALSE, fig.height=2.5}
col <- c('#0000FF', '#00DD00', '#CCCC00')
...
plot(as.raster(elcolor))
# take two directly to png; a total mess, should have done this properly...
```
```{r echo=FALSE, eval=FALSE}
analygraph2 <- function(left, right) {
  analygraph <- array(1, dim=c(dim(left), 3))
  analygraph[,,1] <- left      # red channel
  analygraph[,,2:3] <- right   # green and blue
  analygraph
}
# png('stereoscopic-square.png', width=800, height=800, bg='transparent')
png('stereoscopic-rect.png', width=800, height=600, bg='transparent')
par(bg='transparent', mai=numeric(4))
left2 <- left
right2 <- right
left2[left2==0] <- 1
right2[right2==0] <- 1
elcolor2 <- analygraph2(left2, right2)
elcolor3 <- array(1, dim=c(dim(elcolor2)[1:2], 4))
elcolor3[,,1:3] <- elcolor2
elcolor3[,,4] <- rowSums(elcolor2, dim=2) < 3
plot(as.raster(elcolor3))
dev.off()
```
</div>
