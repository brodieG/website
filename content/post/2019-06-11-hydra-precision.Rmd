...
numbers.  Double precision floating points are typically encoded in binary as
...
For clarity we'll used a mixed binary/decimal representation, where the fraction
is shown in binary, and the exponent in decimal[^ieee-754-approx]:
...
The green portion of the fraction is used to emphasize the precision.  In the
fraction, each "one" dictates that the corresponding power of two is part of the
value.  So for `A.n`, we can approximately compute the value as:

```{r}
2^18 + 2^17 + 2^16 + 2^11 + 2^10 + 2^9 + 2^8 + 2^7 + 2^6 + 2^5 + 2^3
```

We only used the first 15 bits of the fraction for illustrative purposes,
...
fraction[^fifty-three] we can barely tell the two numbers apart.  As a
...
All those extra decimal digits are just an artifact of the conversion, not
evidence of precision.  Conversely, binary double precision floats can struggle
to represent seemingly simple decimal numbers.  For example, 0.1 cannot be
expressed precisely at all with any finite number of bits.  The built-in `iris`
data set makes for a good illustration as most measures are taken to one decimal
place.  This is what it looks like in binary format, with the factor levels in
`Species` converted to their numeric equivalents:

<figure class='aligncenter' style='max-width: 100%;'>
<!--<a href='#image-credits' title='Click for image credits.' class=image-credit>-->

```{r bin_rep, echo=FALSE, fig.height=3}
 # A bit sloppy
...
```
<!--</a>-->
<figcaption>Iris Underlying Binary Representation, Big Endian</figcaption>
</figure>

Colored cells are ones, and black cells zero, blue ones are the exponent, and
green ones the fraction.  The sign bit is the first column and would show up
in yellow if there were any negative numbers.  Let's zoom into the first ten
...
black (zeroes) in the first two rows[^float-exact].  The other numbers cannot
be represented precisely, as is implicit in the use of every available bit.  We
can also see this by showing more digits in the decimals:
...
Subtraction of two nearby floating point numbers is a recipe for disaster, and
unfortunately for us our `cumsum` and `diff` algorithm is likely to cause this.
...
The [80 bit floating point][14] representation adds 10 bits to the fraction.
...
[cumulative sums earlier](#eighty-bit), wonder no more.  For example, for
...
* This floating point representation is a re-creation of [IEEE-754 Double
...
[^float-exact]: Integers of magnitude less than $2^53$ can be represented
  ...
  Similarly a subset of integers larger than `$2^53$` can be represented exactly
