...
two smaller triangles.  The process is repeated until the approximation error is
can be tolerated.
...
We now have a rough idea of the algorithm, but as I learned the hard way this is
grossly insufficient for those of us that intend to mess with it.  You can see
the  [full algorithm in the appendix](#r-transliteration-of-original-algorithm)
or in the [Agafonkin post][6], but the part we want to focus on first is the
mesh triangle identification.  Here is a simplified version that highlights how
the triangle vertex coordinates `$(ax,ay)$`, `$(bx,by)$`, and `$(cx,cy)$` are
computed:
...
Remarkably this will compute the vertex coordinates of every single triangle at
every level of approximation in our mesh, from smallest to largest.  I get a
headache trying to mentally visualize what this code actually does.  Thankfully
with [`watcher`][9] we can let the computer do the heavy lifting.  This
is what happens in finding the first triangle in a 5x5 mesh[^min-complex]:

<div id='flipbook1'></div>

When the target triangle is found it is turns yellow.  After that point the
algorithm computes the location of the midpoint of the hypotenuse, and records
the error at that point.  Since this is the most granular approximation level
there is no child error to propagate[^no-child-error].

This process is repeated with changing initial `id` values until eventually the
entire surface has been tiled with a triangle:

<!--
Show frames 100, 194, 382, 758
-->

At this point something interesting happens.  By its nature the algorithm
repeats the same calculation but stops at the next largest triangle size.
Additionally, since we are no longer at the most granular approximation it also
starts carrying over the child errors.  Here we show the computation on the
second triangle at the next approximation level[^second-triangle]:
...
target point.  Do note that more errors will be carried over.  A few hundred
steps later in the process the other set of children that coincide on this
particular hypotenuse are addressed:
...
## R Transliteration of Original Algorithm

This a reasonably close transliteration of the original [Agafonkin][6]
implementation of the [RTIN error computation algorithm][8].  In addition to the
conversion to R, I have made some (hopefully) non-semantic changes so that the
code is easier to instrument.  This only computes the error.
...
var img_dir = '/post/2019-08-23-mesh-reduction-1_files/images/flipbook/';
var fps_def = 4;
var end_delay = 8;
new BgFlipBook('flipbook1', img_dir, 53, 7, fps_def, end_delay, "0000");
new BgFlipBook('flipbook2', img_dir, 844, 803, fps_def, end_delay, "0000");
</script>
