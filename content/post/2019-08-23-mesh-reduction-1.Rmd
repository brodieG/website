...
(lavender), and the corresponding child hypotenuse midpoints (orange):
...
to_arrows <- function(grp) {
  ends <- grp[2,,drop=FALSE]
  ends.b <- grp[3,,drop=FALSE]
  mid <- grp[1L,,drop=FALSE]
  child <- grp[-(1:3),,drop=FALSE]
...
p2 <- ggplot(tris2) +
  ...
  geom_point(
    ...
    aes(x, y, colour=I('grey35'), fill=I('#8da0cb')),
    shape=21, size=3
  ) +
  geom_point(
    data=subset(p.all, ptype=='ends'),
    aes(x, y, colour=I('grey35'), fill=NA),
    shape=21, size=2
  ) +
  geom_point(
...
Let's look at this in action with the next grid size, 17 x 17:
