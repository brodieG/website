...
layer and collapses them to "linearized" coordinates:
...
Hold on a sec, weren't we going to vectorize the $#!t out of this?  What about
...
Alright, now we're in business.  We're beating neither C nor JavaScript, but
...
some simple changes we can make to double the speed, but we have even grander aspirations.

Before we move on though, a quick recap of some vectorization concepts we
applied here:
...
To achieve bullet two we resorted to carefully structured seed data, and
repeated it explicitly and implicitly with vector recycling.  We did this both
for the actual data, as well as for the indices we used to subset the data for
use in the vectorized operations.

# Indulging in Follies Of Unnecessary Optimization
...
```{r echo=FALSE}
p.ax1 <- p.ax
p.ax2 <- rbind(p.ax1, transform(p.ax1, y=y + .5))

p.ax1[['type']] <- factor('Axis Single', levels=c('Axis Single', 'Axis Column')) 
p.ax2[['type']] <- factor('Axis Column', levels=c('Axis Single', 'Axis Column')) 

p.all <- rbind(p.ax1, p.ax2)
p.all[['grp']] <- (seq_len(nrow(p.all)) - 1L) %% 4L
