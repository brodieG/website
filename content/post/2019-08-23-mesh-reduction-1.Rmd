...
You should look at [&commat;mourner's][5] [notebook][6] or the original [Will
Evans et al. 1997 paper][8] for in-depth details as the point of this post is
the translation to an R idiomatic implementation.  Notwithstanding I can't
resist indulging in some of my newly acquired `rayrender`ing powers for some
illustrations.

The idea is to approximate a surface with heights defined on a regular
grid in the x-y plane with a subset of those coordinates, subject to an error
tolerance.  The beloved `volcano` height map is exactly this type of surface,
and here we show the full detail surface (glass) and several approximations
meshes (metallic) with different tolerances.  The tolerances are specified
as percentages of total height.
...
Look closely (click on the images for full-rez) and you'll see the
approximations crossing in front of and behind the full resolution surface.
From a distance this manifests as localized darkening of the mesh.

When we flatten the elevation component of the meshes it becomes apparent that
all the triangles in the approximation are right angled isoceles triangles on
the x-y plane.  This is where the "Right-Triangulated" part of the algorithm
name comes from.
...
In order to decide whether an approximation is faithful enough we measure the
