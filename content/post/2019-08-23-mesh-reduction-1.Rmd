...
<img
  ...
  data-caption='Seven `volcano` approximations, flattened and stacked.'
...
<div class=bgw-wide-window>
<video
  id=louvre-rotate style='display: block; margin: 0 auto; max-width: 100%;'
...
We keep the approximation mesh color the same in the video to make it easier to
track it across frames, though going forward we go back to different colors
indicating different levels of approximation.
...
algorithm.  It allows us to compute the approximation errors of every triangle
at every resolution first, and then later quickly extract meshes comprising
triangles that meet a particular error tolerance threshold.

For the sake of exposition we will abandon volcano and switch to a simple 3x3
elevation surface to illustrate how the approximation errors are computed:
...
exactly to the elevation surface.  To get to the next level of approximation
(silver) we combine triangles with adjacent right angles.  This causes error to
appear in the middle of the hypotenuses of the resulting larger triangles.
We show these errors as checkered cylinders.  At the coarsest level of
approximation (copper) we again combine triangles with adjacent triangles,
which generates additional error on the center of the surface.
