...
crossings are highlighted with the blue circles.  Ironically, the best looking
...
guarantees apply when we compute the coordinates directly.  Our
vectorization will require much care and some contortions.

A further complication is that for each triangle, we need to compute the
coordinates of its children well so that we may [carry over](#carry-over-viz)
their errors.

There are likely many ways to go about solving this problem, but I chose to
