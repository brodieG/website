...
bug bit me[^susceptible].  It doesn't help that this problem also presents some
interesting visualization challenges, so once again I veered hopelessly
off-course[^off-course] and wrote this post about it.

Our ultimate objective is to reformulate the MARTINI JavaScript algorithm into
something that can run reasonably efficiently in R.  As it stands the reliance
on explicit loops makes it slow in R.  Of course we could also trivially port it
to C and interface that into R, but where's the fun in that?  In this particular
post we'll focus on specific details of the MARTINI implementation and **how**
it does what it does.  This way we will be better placed to take it apart and
put it together again.  Oh, and we'll indulge in some directed computer drawing.
If all you care about is the reformulation you'll need to wait for Part II.
...
We're not going to spend time here evaluating the effectiveness of this method
as our focus is on the mechanics of the algorithm.  That said, it's noteworthy
that the "2%" approximation reduces the polygon count over five-fold and looks
good even without any shading tricks.
...
coincide with surface vertices. This is because the approximated meshes are made
from subsets of the vertices of the original surface.

Unfortunately even when realistically ray traced 3D images projected onto 2D
inevitably lose... depth.  One way we can compensate for this is to present
different perspectives so that our brains interpret the parallax for additional
depth perception.  Sorry `volcano`, I only do this to you about once a year and
it's for a good cause.

<div class=bgw-wide-window>
<video
  id=louvre-rotate 
  style='display: block; margin: 0 auto; max-width: 100%; width: 720px; height: 720px'
...
We keep the approximation mesh color unchanging just in the video to make it
easier to track it across frames.

Let's flatten the elevation component of the meshes to look at their 2D
structure.  All the triangles are right angled isoceles triangles in the
x-y plane.  This is where the "Right-Triangulated" part of the algorithm name
comes from.
...
connecting it to the opposite vertex.  This means that every edge of the
original triangle overlaps with an edge of the smaller triangles.  As we'll see
in the next post this creates an important constraint on how the different
approximation layers are calculated.

# Errors

We've seen that MARTINI allows us to extract approximation meshes subject to a
particular error tolerance, which means that at some point we have to calculate
the approximation error.  We'll see how this is done with a simple
3x3 elevation instead of `volcano` for the sake of exposition:
...
splitting larger triangles in the middle of the long edge.  So at the first
level of approximation (silver) we want to measure the error in the middle of
the long edges, shown here as the grey checked cylinders.  We know we can remove
that error by moving to the more granular level.  At the next level (copper) we
repeat the process.  This time the long edge runs across the middle of the mesh.
...
approximation at overlapping locations.  This means we can store the errors in a
...
Since the errors of each level are measured at different points,
the above matrices contain the approximation errors for ever level of
approximation.  We only need to calculate the matrix once, and thereafter we can
test it against different tolerance thresholds to determine what approximation
mesh to draw.

# Surface Extraction

Let's play through the process of extracting meshes to different tolerance
levels.  The idea is the extract the coarsest mesh that does not have any
remaining errors above the threshold.  Resolving errors is easy: just put a
vertex at any of the surface points where the error is too high.  Since the
vertex will be on the original surface there will be no error there anymore.

Here we illustrate our error tolerance as a "water" level; errors peeking
above the surface must be addressed:
...
corresponding triangle in the middle of the long edge to remove the error.  This
requires us to draw the highest detail triangles for the bottom right part of
the surface (gold), but lets us use the first level of approximation for the top
left.
...
all four children, it just so happens in this case there is only one that has an
error greater than the parent.  Now our mesh extraction can work correctly
...
full mastery of the details (or at least, I would have greatly benefited from
it).

We will focus on the error computation algorithm as that is the more complicated
part of the process.  Its outline follows:
...
`$(bx,by)$`, and `$(cx,cy)$`, which we can use to compute the error at the
...
[^susceptible]: I appear to be particularly susceptible to it.
