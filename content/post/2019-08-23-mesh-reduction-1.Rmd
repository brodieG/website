...
even just run it via [R V8][14]?  For the sport of it, duh.
...
threshold.  Resolving errors is easy: just put a vertex at any of the surface
points for which the error is too high.  That vertex will be on the original
surface and as such have no error.

Let's illustrate our error tolerance as a "water" level; errors peeking
above the surface exceed the tolerance and must be addressed:
...
of approximation, and another 2 at the next level).  The algorithm does not
cycle through the most detailed level as those by definition have no error.  For
each of these triangles, the `while` loop identifies the vertex coordinates
`$(ax,ay)$`, `$(bx,by)$`, and `$(cx,cy)$`, which we can use to compute the error
at the middle of the long edge.
...
help of [`watcher`][9].  Each frame displays the state of the system _after_
the evaluation of the highlighted line:
...
search for the second triangle at the next approximation
...
made many of the illustrations in this post, and [Vladimir Agafonkin][5] for writing
great posts and for introducing me to the RTIN algorithm.
