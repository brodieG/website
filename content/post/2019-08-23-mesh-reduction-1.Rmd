...
Furthermore, the children are located at easily pre-computed offsets from the
midpoints:
...
<!--
Add the midpoints, showing oob children as maybe empty dots
-->

All is not perfect though: "Axis" is again a problem as the peripheral midpoints
end up generating out-of-bounds children, shown as hollow points above.  This is
particularly annoying when we are using ["linearized" coordinates](#linearized)
as it is not trivial to detect whether a child is out-of-bounds or not after
applying offsets.  We could switch back to x/y coordinates and check whether any
coordinate is less than zero or greater than the number of rows/columns, but
that is expensive as it requires full vector scans on every child in addition to
the extra storage for the coordinates.  Instead we handle the peripheral
midpoints separately.  In fact, we handle the leftmost, topmost, rightmost, and
bottommost midpoints each separately.  This makes the code increasingly ugly,
but also fast.  I'll spare you[^spare-you] yet another animation and skip
straight to the benchmark:

<!--
Add link to packaged version in increasingly ugly
-->

Suck it JavaScript[^suck-it]!  And check out what happens when we start making
the maps large:
...
We even beat C!  At the lower sizes the overhead of the R calls is hold us back,
but once the maps get large enough the more efficient vectorized R
implementation takes the lead.




tiles as the midpoints in the "Diagonal"
tiles are in the inside of the tile, so one simple optimization we could effect
is to only order errors for "Axis" tiles.  We can even skip the ordering for the
first layer of "Axis" tiles that doesn't have children, and
that one is the most expensive.

We can more efficiently h
...
[^suck-it]: When I say this I'm completely relying on no one else being neurotic
