...
alone is scoff-worthy enough that I deserve little sympathy for the plight I
find myself in[^plight].
...
* As a result, `order` became 50x faster under some work loads.

I was also alluding that something fairly remarkable happens when you order data
before you process it.  Let's illustrate with 10MM items in ~1MM groups: <span
id=working-set></span>
...
memory latency.  What is microarchitecture?  Imagine you have a room full of
children at a sleepover, and as it is past midnight you have firmly instructed
them to sleep.  When you open the door to check on them they are all in their
beds, eyes screwed shut.  Never mind that the moment you close the door and walk
down the stairs the raging pillow fight resumes unabated.  Similarly the CPU is
free to implement assembly instructions[^assembly] however it wants provided
that when the program interacts with it the CPU state visible to the
program is consistent with its instructions.

A more sober analogy: microarchitecture is to assembly instructions as a
compiler is to a C program[^c-to-assembly].  I am inclined to the less sober
analogy because once upon a time I used to think writing C code was being close
to the "metal".  I'm realizing instead that writing C to be close to the metal
is like dwelling in the sewer system to be close to the iron core of the
planet[^unfair-to-c].  If I'm being over-dramatic it's because I'm still a
little pissed at how little control I have over my program's execution, and more
so that that is likely a good thing.

Main memory latency is terrible, but thankfully its bandwidth has kept pace with
processor speeds.  The micro-architectural freedom chip designers have allows
them to deploy tricks to take advantage of bandwidth to mask latency.  The
over-used analogy is that of a long conveyor belt connecting a storage warehouse
(main memory) to the factor (CPU).  The first item requested from the warehouse
will be slow to arrive (latency), but if the conveyor belt is loaded efficiently
it will keep the factory fully supplied from the moment the first item arrives
(bandwidth).

There are two main classes of micro-architectural tricks used to mask the main
memory latency.  The first involves having some amount of low latency memory
known as cache.  In our conveyor belt analogy it might be a shelf next to the
assembly station with room for a few items.  This memory is very limited because
it requires a more expensive design _and_ it must be close to the CPU, typically
on the exclusive real estate of the CPU die:
...
access patterns that defeat them.  `split`ting the randomly ordered vector is an
example of this.  [`treeprof`][22] gives us detailed timings for the random and
sorted `split` calls[^treeprof-manip]: <span id=split-times></span>
...
Our conveyor belt analogy can give us some idea of the type of problems
caused by random inputs.  Suppose the factory places a request for items from
the warehouse, taking care to order the items in the request form.  This is what
the request will look like mid-process, with processed items in yellow:
...
In both cases the first package will arrive at the same time after the full
access latency, but in the first case we will be able to ship data at a much
higher average rate.
...
In our warehouse example we implicitly assumed that the working set was too
large to fit in cache.  If it had fit in cache, we could have drawn our packages
directly from the local storage in the factory with no need to resort to the
warehouse and long conveyor belt.  We're interested in verifying that cache is
effective at mitigating memory system latency.

My system has a three level cache hierarchy as in the [pictured die](#cpu-die).
...
Unlike other levels, L1 has two types: data, and instruction[^l1-data-vs-inst].
As our workload is data rather than instruction heavy we focus on the data
cache.

Given the cache structure of my system we can we can build a model of memory
access time by working set size.  If cache is effective we would expect average access times to follow our model. For reference we show our [current working
set](#working-set) size as a vertical dashed line[^working-set-size]:
...
at 24 cycles per element in both cases[^access-dominates].

Of the six accesses four will always be sequential.  The other two are
sequential for the sorted set, and random for the random set.  We assume that
random accesses are subject to the full memory system latency, the magnitude of
which will vary depending on what level of cache the working set fits in.
Conversely we assume the sequential accesses can be met at L1 cache speeds
without actually consuming cache.  This is because as in our conveyor belt
analogy workers can fully use the main memory bandwidth when they don't have to
hunt around for the packages.  Only the first element is subject to the
latency and that is amortized over the rest of the process.  In memory systems
this is implemented as a combination of what is known as prefetching[^prefetch]
and stream buffers[^streams].

For the random set, accesses slow down with working set size as an increasing
proportion of accesses come from slower memory without the benefit of
prefetching/streaming.  Each transition to a slower level of memory is manifests
as an increase in per-element time that asymptotically[^asymptotic-latency]
approaches the slower memory's incremental latency[^access-anomaly].

We can time `split.default` across a range of working set sizes to compare to
our model:
...
works remarkably well across variety of working set sizes.  It predicts
a ~13x slowdown in the random vs. sorted case for the 10MM element working set,
which is close to the [earlier observed ~15x](#split-times).  Since we are only
modeling cache effects and the timings line up so closely, this implies that
there are no other optimizations in effect[^model-caveat].
...
memory accesses up to this point were sequential, so executing out-of-order is
of marginal value as subsequent reads would be fast anyway.  But the
writes to `res` are random so subject to the full main memory latency, assuming
the working set does not fit in cache.  Out-of-order execution allows us to run
them both concurrently.  This is of value because main memory has substantial
parallelism built-in[^mem-parallelism].  In our analogy the equivalent is
multiple workers available to load (or unload) the conveyor belt at different
stations.
...
cache-only model, particularly once the working set exceeds L3 cache size.

Sure enough, it turns out that the `split.default` code has a sequential
dependency: it uses an offset vector to track where in each group vector to
write the next value, and this vector is updated in the loop[^loop-dep].
...
systems that it's easy to forget the whole thing is a massive hack.  We are
stuck between slow commodity memory on one hand, and the expensive, limited, and
complex micro-architecture that makes it useful on the other.
...
processes.  The combination has led to an entirely new class of security
vulnerabilities that will be difficult or impossible to fix in existing
processors.

The bottom right icon corresponds to the [MDS vulnerability][82] announced while
I was in the process of writing this post.  What is staggering is that the
changes that Intel introduced to its processors to mitigate the top left
vulnerability discovered over a year ago ([Meltdown][83]) have made some of MDS
...
The joke in all this is that the micro-architectural mess we just slogged
through is a side-show.  I care about ordering data not because of the cache
and other benefits, but because ordered data opens up possibilities for
different and more efficient processing algorithms, independent of
micro-architectural factors.  It's just that when I noticed the
micro-architectural factors wagging their tongues in my face I felt
compelled to understand what was going on.

So, stay tuned for Part II of the Hydra Chronicles, in which we will explore how
ordered data can do amazing things.  Originally I was planning on ending the
post with the hydra drawing minus a couple of heads, but its kinda grown on me
and I feel bad doing that.  Henceforth it will be a mascot and the name of this
series of blog posts, and it won't make any sense except to those like you that
read the first one.  That's okay, it will be our inside joke.
...
[^c-to-assembly]: In both cases a process will interpret a set of instructions
  (C code, assembly) into a different lower-level set of instructions
  (assembly, CPU micro-operations).  The processes must abide by constraints (a
  flavor of the C standard, an [Instruction Set Architecture][99]), but these
  are loose enough to allow outrageous re-interpretation.  There are few things
  quite as humbling as learning just enough assembly that you can look at the
  disassembly of what you thought was a simple C function and realize there is
  almost no relationship between what you wrote and what the program is doing
  other than the semantics are the same.   Well, they should be provided you did
  not have the misfortune of using unintended undefined behavior.
