...
Here we converted the for loop into a repeat / break conditional loop, which is
really what the for loop is anyway.  In order for the processor to initiate
future loop iteration calls, it must make an assumption about whether there will
indeed be a next iteration.  If it turns out to be wrong, all the operations
...
crazy_add2 <- inline::cfunction(
  sig=c(x='integer', y='logical', res='integer'), body="
  if(XLENGTH(x) != XLENGTH(y)) error(\"invalid inputs\");
...
crazy_add <- inline::cfunction(
  ...
  for(i = 0; i < len; ++i) {
    if(y_ptr[i] > 0){
      ...
      res_ptr[i] = x_ptr[i] + 1 + y_ptr[i];
    }
  }
  return(res);
")
sane_add <- inline::cfunction(
  sig=c(x='integer', y='logical', res='integer'), body="
  if(XLENGTH(x) != XLENGTH(y)) error(\"invalid inputs\");
  if(XLENGTH(x) != XLENGTH(res)) error(\"invalid inputs\");
  R_xlen_t i, len = XLENGTH(x);
...
[^hash-tables-in-r]: Hash tables used to take up 4x the size of the hashed
  input, but in [2004 that was switched to 2x][65].  A tidbit I found
...
[^rob]: Assuming no instruction dependencies, CPUs that can engage in
  ...
  three instructions per iteration, in reality there will be far more
  micro-operations because the actual R code is [more][31] [complex][60]
  than the pseudo code would indicate.
