...
memory latency.  Micro-architecture is The Implementation Detail.  Higher level
programs like C are compiled into [machine instructions][99].  In simpler times,
machines would just follow those instructions.  Nowadays modern CPUs just
pretend to do that.  Behind the scenes, they re-interpret them in whatever way
they see fit[^assembly-is-to-c].  The only constraint is that the CPU state
visible to the instructions must be consistent with them.  "Behind the scenes"
anything goes, so to speak, including some hair raising optimizations.
Everything that happens "behind the scenes" is the micro-architecture.
...
sequential.  When they are not two of those are no longer sequential.  In the
model we assume sequential accesses can be met at L1 speed without consuming
cache.  This is an example of where memory systems can use of bandwidth to
mitigate latency, and is typically implemented as a combination of
prefetching[^prefetch] and stream buffers[^streams].  In the conveyor belt
analogy this would be approximated by requests always being made from the same
station.

Additionally, we assume that random accesses are subject to the full memory
system latency, the magnitude of which will vary depending on what proportion of
the working set fits in each level of cache.  Each transition to a slower level
of memory manifests as an increase in per-element time that
...
this effect quickly dissipates as working set size increases.
...
A pseudo code version of the [subset C code][31] might look like:
...
loop](#pseudo-random) into the following instructions:<span
id=linear-ops></span>
...
built-in[^mem-parallelism].  In our analogy the equivalent is multiple workers
available to load (or unload) the conveyor belt at different stations.

In this example we would be able to meet both random accesses in the same time
as one[^mem-parallel-caveat].  Processing speed for this type of access will
grow with main memory parallelism and our ability to put more instructions
"in-flight".
...
instead we wrote a [C function to do it](#index-chasing).
...
Adding the sequential dependency brings timings much closer to the theoretical
...
over the past two years that all exploit micro-architectural optimizations.
...
recent vulnerabilities, but is enough to compel me to comment.  If you have been
reading the headlines over the past year or so and have wondered exactly what
this was all about, I encourage you to read the papers.  I've read the
[Spectre][84] and [Fallout][85] papers which are remarkably accessible, and
occasionally [downright funny][86].
...
* [Westmere 6 Core][102], posted by redstern, license unknown.
...
[^assembly-is-to-c]: Another way to think about is that micro-architecture is to
  ...
  are the same.  Well, they should be provided you did not have the misfortune
  of using unintended undefined behavior.  I'm realizing that writing C to be
  close to the metal is like dwelling in the sewer system to be close to the
  iron core of the planet.  If I'm being over-dramatic it's because I'm still a
  little pissed at how little control I have over my program's execution, and
  more so that that is likely a good thing.  Don't get me wrong, I actually
  quite enjoy writing C code and undoubtedly you have way more control than you
  do from even higher level languages.
[^cache-line]: Cache line are typically 64 byte blocks, which for a numeric
  vector would correspond to eight elements.  Cache lines are themselves a form
  of prefetching as they only make sense under the assumption that you are
  likely to need the elements adjoining a requested memory location.
[^unfair-to-c]: Clearly this is an unfair characterization, but there is enough
  truth to it that I feel entitled to the poetic license.  I actual quite enjoy
  (usually) coding in C, and you do have way more control of things than you do
  with R or other even-higher-level languages.
