...
this is happening because of out-of-order execution and memory interleaving /
...
step[^rob], and how many different memory banks our requests
points[^mem-parallelism].  In this case the out-of-order execution and
parallelized memory access allows the vector re-order to be carried out
substantially faster than the cache-only model would suggest[^ooo-interleave].

# Out of Order Execution

We can visualize the out-of-order execution in a system that allows up to twelve
instructions in flight simultaneously as follows.  Again we use some
pseudo-R-code as stand-ins for what would actually be machine
...
val3      <- x[i3]                 # pending
idx3      <- o[i3]                 # pending
res[idx3] <- val3                  # pending
```

This corresponds to the first three iterations of the loop.  For convenience we
will pretend there are only three loop iterations in total.  Due to the code
dependencies, the actual execution order of the instructions will look something
like:
...
Notice how I renamed the `i`, `val`, and `idx` variables by appending the loop
number to each of them.  This allows us to keep multiple `i` values in flight
without later ones clobbering earlier ones.


With a large enough
working set everything is going to be coming from main memory, so 


One way to confirm that out-of-order execution is indeed happening we can
compare with a similar work load where there is a loop iteration dependency.  We
...
This is effectively equivalent to pointer chasing.  Unfortunately it isn't
possible to engage in such index chasing efficiently in pure R code, so instead
we wrote a C function to do it.  The code to generate the "chasing indices" as
well as the C logic to "walk" them is [in the appendix](#index-chasing).
...
Sure enough, adding the sequential dependency brings timings much closer to the
theoretical cache-only model, particularly once the working set exceeds L3 cache
size.  Random accesses are somewhat pathological to modern memory systems, but
those that depend on prior accesses are downright killer.  Ordering our input
vectors first isolates random accesses to the simplest operations, thereby
giving the memory system optimizations the best chance to mitigate main memory
latency.

# Branch Prediction

One optimization we glossed over is branch prediction which is used to attempt
to resolve a class of sequential dependency that would otherwise limit how far
ahead out-of-order execution could get.  We can re-write our [subset pseudo
code](#subset-pseudo) to emphasize the issue:
...
We converted the for loop into a repeat / break conditional loop, which is
really what the for loop is anyway.  In order for the processor to be able
to out-of-order execute multiple loop iterations it needs to decide whether the
condition that triggers the break statement will be met or not.

Suppose that our CPU can have up to ten operations in flight:
...
While it's
obvious to anyone looking at the

If it guesses
wrong we will suffer a ~20 CPU cycle penalty.  If it guesses right we get to
execute out-of-order and take advantage

be met with a penalty of about 20 CPU cycles.


initiate
future loop iteration calls
, it must make an assumption about whether there will
be an additional iteration or whether it should execute the operation that
follows the loop.
.  If it turns out to be wrong, all the operations
that were initiated out of order past the termination point will have to be
thrown out, and the processor will stall while the correct instructions are
loaded.

In the case of a long for loop as we have here processors will do very well just
by assuming that the loop will be repeated.  Eventually they will be wrong when
the loop terminates, but the cost of the incorrect branch prediction at the end
is well amortized over the rest of the loop.

```{r eval=FALSE}
#include <stdlib.h>
#include <stdlib.h>
int *crazy_add(int *x, int *y, int size) {
...
int *crazy_add(int *y, int size) {
  int * res = (int*) malloc(size * sizeof(int));
  int accum = 0;
  for(int i = 0; i < size; ++i) {
    if(*y > 0) {accum++;} else {accum--;}
    *res = accum;
    ++res;
    ++y;
  }
...
crazy_add3 <- inline::cfunction(
  sig=c(x='logical', res='integer'),
  body="
  if(XLENGTH(x) != XLENGTH(res)) error(\"invalid inputs\");
...
y <- sample(c(TRUE, FALSE), 1e7, rep=TRUE)
y2 <- logical(1e7)
z <- 1:1e7 + 1L - 1L
res1 <- integer(1e7)
res2 <- integer(1e7)
...
microbenchmark::microbenchmark(
  .Call('crazy_val', y, res1),
  .Call('crazy_val', y2, res2)
  times=20
)
sys.time(sane_add(z, y, res2))
```
