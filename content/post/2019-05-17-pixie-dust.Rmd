...
# In Which it Dawns On Us That We Know Nothing
...
Three months ago I reached a fork in the road and I could have taken the "oh,
...
with my flailing efforts to cut off its heads.
...
memory access is laggy.  Over the past few decades CPU processing speed has
...
them to sleep.  Sure enough when you open the door to check on them they are all
in their beds, eyes screwed shut.  Never mind that when you close the door and
walk down the stairs the raging pillow fight resumes unabated.  Similarly the
CPU is free to implement assembly instructions[^assembly] however it wants
provided that when the program interacts with it the CPU state visible to the
program is consistent with its instructions.

A more sober analogy: microarchitecture is to assembly instructions as a
compiler is to a C program[^c-to-assembly].  I am inclined to the drunker
...
them to develop tricks to take advantage of the bandwidth.  The over-used
analogy is that of a long conveyor belt connecting a storage warehouse (main
memory) to the factor (CPU).  The first item requested from the warehouse will
be slow to arrive (latency), but if the conveyor belt is loaded efficiently
...
the second class of optimizations (better loading of conveyor belt) that we will
...
access time by working set size.  For reference we show our [current working
...
sequential only for the sorted set.  Sequential accesses are important because
they are easy to predict.  This is one of those ["load the load conveyor belt"
optimizations](#second-trick).  In the analogy workers notice the sequential
pattern and pack the conveyor belt with the subsequent but as of yet unrequested
boxes.  The memory system does the same.  We assume that all sequential accesses
can be met at L1 cache speeds after the first element without consuming cache.
This optimization is likely some combination of prefetching[^prefetch] and
stream buffers[^streams].

The random set cannot benefit from this optimization as the access is not
predictable.  When the working set exceeds L1 cache, accesses slow down as an
increasing number of them must come from slower memory without streaming.  Each
transition to a slower level of memory is manifests as an increase in
per-element time that asymptotically[^asymptotic-latency] approaches the slower
memory's incremental latency[^access-anomaly].
...
works remarkably well.  This implies that none of the tricks other than cache
are having a substantive effect on performance[^model-caveat].
...
cache-only model.  Most likely this is happening because of out-of-order
execution enables the memory system to use the parallelism in main memory.
...
iterations without waiting for the first one to complete.  Imagine having
multiple workers feeding the conveyor belt.  We could instruct each of them to
find a different random item in the warehouse.  Enough workers would be able to
pack the conveyor belt fully even if each individual worker was slow to find
the requested item.

We can amortize the memory latency cost by the lesser of how many loop
iterations we can put "in-flight" out-of-order, and the degree of parallelism
(warehouse workers) available in main memory[^mem-parallelism].
...
"in-flight".  The first two iterations of the [previous `for`
loop](#pseudo-random) are turned into the following instructions:<span
id=linear-ops></span>
...
writes to `res` are random so subject to the full main memory latency.  Out of
order execution allows us to run them both concurrently which allows the memory
system parallelism to satisfy them both in roughly the same amount of time as it
would take to run a single one[^mem-parallel-caveat].  [Without out-of-order
execution](#linear-ops) the second write to `res` would wait for the first one
to complete.  Processing speed for this type of access will grow with main
memory parallelism and our ability to put more instructions "in-flight".

One way to confirm that out-of-order execution is indeed happening is to prevent
it by adding a sequential iteration loop dependency for an otherwise similar
work load. We can use "chasing indices" like this one:
...
[^unfair-to-c]: Clearly this is an unfair characterization, but there is enough
  truth to it that I feel entitled to poetic license.  I actual quite enjoy
