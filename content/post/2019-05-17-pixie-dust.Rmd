...
flailing attempt at chopping off one its heads.
...
play first hand, although branch prediction proper is a minor element in this
case.

> **Disclaimer**: Before I started writing this post all I knew about
> micro-architecture is what I could infer from its constituent words,
> particularly the 80s-90s marketing hype around the first one.  My only
> expertise is that my relative lack of knowledge of the topic might make my
> writing about it more accessible. I cannot guarantee the accuracy of all
> the details in this post, but the broad outlines should be mostly correct.
...
storage warehouse (main memory) to the factory (CPU).  The first item requested
...
# Micro-architecture To The Rescue?

Chip designers have engaged in micro-architectural heroics to conceal main
memory latency.  Micro-architecture is _The Implementation Detail_.  Higher
...
the instructions, but that leaves room for some truly hair raising
optimizations.  Everything that happens "behind the scenes" is the
micro-architecture.

There are two main classes of micro-architectural tricks used to mask the main
...
main memory latency.  Unfortunately it is often the case that the working set
does not fit in cache.  We're interested in exploring how cache and working set
size affect the speed of the `split.default` calculation.
...
line[^working-set-size], the access time models as colored lines, and observed
timings as dots:
...
buffers[^streams].  We assume sequential accesses can be met at L1
cache speeds without consuming cache.  Conversely random accesses do not benefit
from these optimizations.   We assume they are subject to the full memory
system latency, the magnitude of which will vary depending on how much of the
working set fits in each level of cache.  Transitions to a slower level of
memory manifests as an increase in per-element time that
asymptotically[^asymptotic-latency] approaches the slower memory's
...
with ordered than unordered input.  What this doesn't explain is why the
combination of ordering the inputs and splitting them is faster than splitting
them directly.  After all the ordering process itself involves random memory
accesses:
...
Notice how we renamed the `i`, `val`, and `idx` variables by appending the loop
number to each of them[^tomasulo].  This allows the CPU to re-order them without
instructions from later iterations overwriting earlier ones.  For example, they
could now be executed in the following order:
...
This is where out-of-order execution really shines.  All memory accesses up to
this point were sequential, so executing out-of-order is of marginal value as
subsequent reads would be fast anyway.  But the writes to `res` are random so
subject to the full main memory latency, assuming the working set does not fit
in cache.  Out-of-order execution allows us to run them both concurrently.  This
is of value because main memory has substantial parallelism
built-in[^mem-parallelism], i.e. the warehouse has multiple workers available
to load (or unload) the conveyor belt.  Processing speed for this type of access
will grow with the degree of main memory parallelism and the size of the
"in-flight" instruction window[^mem-parallel-caveat].

One way to confirm that out-of-order execution is indeed happening is to prevent
it by adding a sequential loop dependency to an otherwise similar work load. We
...
With the sequential dependency timings are back to the theoretical
cache-only model, demonstrating that the random subset likely benefited
substantially from out-of-order execution.
...
`+1` and FALSE values as `-`.  We can test it function with logical vectors with
...
The limitations of the micro-architecture expose its supposedly hidden
...
micro-architecture is involved and the timing of the disclosures of the most
...
For those mildly curious, most of the vulnerabilities are similar in spirit to
the following pattern, again in pseudo-R code:

```{r eval=FALSE}
try({
  evict_cache()
  slow_no_op()
  val <- illegal_acccess()   # executed speculatively, read 1 byte
  data_array[val]            # executed speculatively
})
timings <- lapply(1:256, function(x) system.time(data_array[x]))
```

`illegal_access()` should fail, but because of speculative execution the CPU
will run it and  `data_array[val]` while it waits for `slow_no_op()` to
complete.  The two speculatively executed steps are not observable to the
program, but because they _are_ run the value of `data_array[val]` will be in
cache.  After the error is trapped, we can time how long it takes to load
`data_array[x]` for different values of `x`. The value of `x` that loads quickly
from cache is `val` from the speculative run.  Rinse and repeat for different
illegal accesses.
...
* Ulrich Drepper, ["What Every Programmer Should Know About Memory"][27], 2007:
  a comprehensive of modern memory systems, with an emphasis on main memory and
  cache.
* Ed Jorgensen, ["x86-64 Assembly Language Programming with Ubuntu"][200], 2019:
  If you are interested in exploring micro-architecture further, you'll want at
  least a very basic understanding of assembly language.  This is a great
  introduction you can skim through reasonably quickly to get a sense of what's
  what.
* R. M. Tomasulo, ["An Efficient Algorithm For Exploiting Multiple Arithmetic
  ...
  allow out-of-order execution to better utilize the multiple execution units
  ...
  but out-of-order speculation past branches was not.
* Norman Jouppi, ["Improving Direct-Mapped Cache Performance by the Additionof
...
* [Intel 14nm Skylake][88], by [Fritzchens Frits][102],
  under CC0 Public Domain, and [Asus Motherboard][90], by Asus PR.
* [Meltdown][83], [Spectre][84], and [Foreshadow][] logos by [Natascha Eidl][103].
* [MDS][82] logo by [Marina Minkin][105], under CC0.
...
[^assembly-is-to-c]: Another way to think about is that micro-architecture is to
  assembly instructions as a compiler is to a C program.  In both cases a
  ...
  are the same, assuming no unintended use of undefined behavior.
...
[^bp-minor]: There are a few spots where branch prediction is likely happening.
  First in the loops proper as the break condition is a condition.  Since our
  ...
  run quickly, including the loop condition calculation, so the branch will not
  be the bottleneck and thus the value of branch prediction is limited here.
...
[^subset-order]: With working set less than L1 in size, it looks like the system
  ...
  TLB ([Translation Lookaside Buffer][29]) effect, although it seems too early for
  that to happen as the DTLB supports 64 4KB pages, and with a 32KB working set
  (`xo`), the total memory we're using is 32KB + 32KB + 16KB (`xo` + `x` + `o`).
  The next jump at 64MB could be the same DTLB jump we [saw earlier](#anomaly).
...
[102]: https://www.flickr.com/photos/130561288@N04/
