...
Random accesses are somewhat pathological to modern memory systems, but random
memory accesses that depend on prior accesses are downright killer.  Ordering
our input vectors allows us to avoid truly pathological outcomes by randomly
accessing our data without prior access dependency.
...
We can then "walk" these indices with some C code.  Note the result vector is
modified by reference.  This was not really necessary but for some reason I
wanted to run timings excluding the allocation of the result vector.  We write
effectively meaningless but iteration dependent values to the result vector to
try to avoid any clever optimizations from the memory system.

```{r eval=FALSE}
walkrand <- inline::cfunction(
  sig=c(x='numeric', times='integer', res='numeric'),
  body="
...
scanrand <- inline::cfunction(
  sig=c(x='numeric', times='integer', res='integer'), 
  body="
...
ftest <- function(size, reps) {
  if(!is.numeric(reps) || length(reps) != 1 || is.na(reps) || reps < 1)
    stop("bad input reps")
  x <- sample(as.integer(size)) - 1L
  fint(x, as.integer(reps))
}
sizes <- 2^(10:27)
times <- pmax(2^24 / sizes, 1)
sizes.sub <- seq_along(sizes) # tail(seq_along(sizes), 3)
samps <- lapply(sizes[sizes.sub], rand_path)
ress <- lapply(sizes[sizes.sub], numeric)
y <- 10
...
for(i in seq_along(samps)) {
  ...
  time.normal <- sys.time(walkrand(samps[[i]], times[[idx]], ress[[i]]), y)
  time.sorted <- sys.time(scanrand(samps[[i]], times[[idx]], ress[[i]]), y)

  res <- append(
    res, list(
      normal=time.normal, sorted=time.sorted, n=sizes[[idx]], times=times[idx]
  ) )
  print(time.normal)
  print(time.sorted)
}
