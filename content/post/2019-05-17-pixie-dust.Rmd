...
memory access is laggy.  Over the decades CPU processing speed has
...
memory latency.  What is micro-architecture?  Imagine you have a room full of
...
provided that when the program interacts with it the visible CPU state is
consistent with the instructions.

A more sober analogy: microarchitecture is to assembly instructions as a
compiler is to a C program.

Unlike with latency, main memory bandwidth has kept pace with processor speeds.
The micro-architectural freedom chip designers have allows them to develop
tricks to take advantage of the bandwidth.  The over-used analogy is
that of a long conveyor belt connecting a storage warehouse (main memory) to an
assembly station (CPU).  The first item requested from the warehouse will be
slow to get to you (latency), but if the conveyor belt is carefully loaded
you'll have your hands full as soon as the first arrives (bandwidth).
...
utilized after the first item is requested.  This includes preloading memory
areas the CPU is likely to need based on prior access patterns, executing
operations out-of-order so that memory requests for later operations are
initiated while earlier ones wait for theirs to be fulfilled, guessing at which
conditional branch will be executed before its condition is known, and other
optimizations.
...
complex micro-architecture required to make it useful on the other.  The
limitations of the micro-architecture expose its supposedly hidden
