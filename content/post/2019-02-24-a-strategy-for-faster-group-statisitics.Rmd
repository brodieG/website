...
imagerect: /front-img/strategy-for-groups-wide.png
imagemrgvt: 0%
imagemrghz: 20%
weight: 1
contenttype: article
...
Mostly this is not an issue because the key workhorse functions hand off quickly
to lean pre-compiled routines.  We must be careful though when we use R-level
loops such as `for`, or `*pply` family[^for-ply] and similar[^purrr].   So long
as the number of iterations is small relative to the data size, there is no
problem.  For example, looping through the _columns_ of a 1,000,000 row x 10 column
data frame is fine, but looping through the _rows_ less so.

A common use case where this limitation is difficult to avoid is the computation
of group statistics on data sets with many groups[^sac-on-so].  This will
requires a minimum of one R function call for each group, <span
id='split-bad'></span>and the additional overhead of splitting the input into
R-level groups.
...
that as follows, with individual colored squares representing vector values and
...
Adding the unary `+` disables "GForce":
...
`data.table` makes this type of computation easy:
...
```
```{r eval=FALSE}
all.equal(res.slope.base, res.slope.dt1[[2]], check.attributes=FALSE)
```
```
[1] TRUE
```
...
currently implement "GForce" within `:=`][12], so the first `keyby` call shown
again below does not benefit from it:

```{r eval=FALSE}
DT[, `:=`(ux=mean(x), uy=mean(y)), keyby=grp]
```

There is a small performance gain, but it is not inherent to this
method[^false-gain]:
...
To work-around this issue we first compute `$\bar{x}$` and `$\bar{y}$` values
into a separate table, which will use "GForce":
...
application where at first blush one might assume it is not possible.
...
an S3 generic closure, which adds R-level S3 dispatch that is slower
than the corresponding compiled code level S3 dispatch, particularly when the
