...
h.c <- cbind(x=c(row(mat)), y=c(col(mat)), z=c(mat))
...
h.c.r <- h.c %*% t(rot_mx(rot.ang))

## It is possible that after rotation order along the x-axis has changed due to
## height differences, so we need to get the cummmax per row.  Also do it by
## col, but it really shouldn't matter there

h.c.r[,1L] <- ave(h.c.r[,1L], c(col(mat)), FUN=cummax)
