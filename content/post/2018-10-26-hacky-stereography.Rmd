...
XR <- rotationMatrix(  40 / 180 * pi, 1, 0, 0)
...
umx <- t(XR) %*% t(ZRl)
umx[4,4] <- 1
rgl.viewpoint(userMatrix=umx, type='modelviewpoint', zoom=.7)
...
empty.rows <- which(
  (rowSums(left) == 3 * ncol(left)) & (rowSums(right) == 3 * ncol(right))
)
empty.cols <- which(
  (rowSums(colSums(left)) == 3 * nrow(left)) &
  (rowSums(colSums(right)) == 3 * nrow(right))
)
