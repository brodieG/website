...
dims.y <- dims.y[zord]
dims.xy <- dims.x * dims.y
bb.id <- rep(seq_along(dims.x)[zord], dims.xy)
x.lens <- rep(dims.x, dims.y)
x.off <- sequence2(x.lens)
y.off <- rep(sequence(dims.y), x.lens)
...
But first we need to set up our vertex data so that we can use it in vectorized
calculations.  Unfortunately, this means that we have to copy the vertex x-y
coordinates for every single point.  This is wasteful, but one of the
trade offs often made in R: use more memory in order to make fewer expensive R
function calls.

```{r}
mesh.tri
## Each id in `bb.id` references a mesh triangle
cols <- c('x','y','t')  # don't need Z vals as `bb.id` already Z ordered
mesh.tri.all <- lapply(mesh.tri[,cols], '[', bb.id)
dim(mesh.tri.all) <- dim(mesh.tri[,cols])
dimnames(mesh.tri.all) <- dimnames(mesh.tri[,cols])
mesh.tri.all
```

```{r}
bc <- barycentric(list(x=p.x, y=p.y), mesh.tri.all[,c('x','y')])
inbounds <- Reduce('&', lapply(bc, '>=', 0))

p.raster <- matrix(numeric(0), x.width, y.width)
p.raster[cbind(p.x[inbounds], p.y[inbounds])] <-
  rep(texture, dims.xy)[inbounds]
plot(as.raster(flip(p.raster)))
```
