...
# plot(as.raster(flip(rescale(volcano))))

par(mai=numeric(4L), xaxt='n', yaxt='n', xaxs='i', yaxs='i')
plot.new()
plot.window(c(0,1), c(0,1), asp=ncol(volcano)/nrow(volcano))
points(
  x=rescale(row(volcano)), y=rescale(col(volcano)),
  col=gray(rescale(volcano)), pch=15
)
```
...
plot.new()
polygon(rescale(x4), rescale(y4), col='#EEEEEE')
```
...
persp(
  ...
  theta=-65, phi=70, col=gray(shadow[-1,-1]), r=1e6,
...
par(mai=numeric(4))
plot.new()
polygon(rescale(c(0, 1, 1, 0), .9), rescale(c(0, 0, 1, 1), .9), col='#EEEEEE')
text(
...
As hinted up to we're looking to make [analygraphs][8] and [stereograms][9].  We
can easily render two scenes with slightly different angles.  For convenience,
we use `shadow::mrender_elevation` which is just a wrapper to `render_elevation`
...
elren <- mrender_elevation(
  volcano, shadow, list(rot.l, rot.r), res=1000, d=125, fov=75
)
left <- flip(elren[[1]])
right <- flip(elren[[2]])
elcolor <- array(0, dim=c(dim(left), 3))
elcolor[,,1] <- left
elcolor[,,2:3] <- right

par(bg='black', mai=c(.25, 1, .25, 1))
plot(as.raster(elcolor))
```

Or we can juxtapose them to make a stereogram:
...
```{r eval=FALSE}
# rot.360 <- lapply(0:36, function(x) rot_z(10*x))
# rot.360 <- lapply(c(0,9,18,27), function(x) rot_z(10*x))
rot.360 <- lapply(0:71, function(x) rot %*% rot_z(5*x))
# rot.360 <- lapply(24, function(x) rot %*% rot_z(10*x))
elren <- mrender_elevation(volcano, shadow, rot.360, res=600, d=200, fov=60)
for(i in seq_along(elren)) {
  png(
    sprintf('test/volc%02d.png', i),
    width=nrow(elren[[1]]),
    height=ncol(elren[[1]])
  )
  par(mai=numeric(4))
  plot(as.raster(flip(elren[[i]])), asp=1)
  dev.off()
}
```
...
The entire 3D rendering pipeline we
...
persp3d(
  ...
  z=c(volcano), color=gray(shadow), lit=FALSE, aspect="iso",
...
# next3d()
# persp3d(seq_len(nrow(mx)), seq_len(ncol(mx)), c(mx),
#   color=rgb(c(shadow), c(shadow), c(shadow)), lit=FALSE,
#   xlab="X", ylab="Y", zlab="Z", box=FALSE, axes=TRUE
# )
# view3d(0,0)
