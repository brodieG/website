...
vlpt <- vlp
vlpt[['x']] <- scale(vlpt[['x']], x.width - 1L, 0) + 1
vlpt[['y']] <- scale(vlpt[['y']], y.width - 1L, 0) + 1
```
...
treeprof::treeprof(
  render_elevation(volcano, shadow, rot, resolution=600, d=.5, zord='pixel')
)
```
```
Ticks: 2888; Iterations: 8; Time Per: 574.6 milliseconds; Time Total: 4.597 seconds; Time Ticks: 2.888

                                          milliseconds
render ------------------------------- : 574.6 -   0.0
    rasterize ------------------------ : 572.2 -   8.2
        mesh_expand ------------------ : 147.0 -   0.0
        |   lapply ------------------- : 147.0 - 147.0
        barycentric ------------------ : 126.1 - 126.1
        shade ------------------------ : 109.6 -   0.0
        |   Reduce ------------------- : 109.6 -   0.2
        |       Map ------------------ :  97.9 -   0.0
        |       |   mapply ----------- :  97.9 -   0.0
...
and perspective adjustments those virtually no time.  So why bother with
...
plot(as.raster(flip(shadow2)))

system.time({
  rel <- render_elevation(elev2, shadow2, rot_z(-90) %*% rot_y(-10), 600, 1,
  zord='pixel')
  plot(as.raster(flip(rel)))
})
system.time({
  mesh2 <- elevation_as_mesh(elev2, shadow2, rot_z(-90) %*% rot_y(-10), d=1)
