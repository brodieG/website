...
system.time({
  rel <- render_elevation(elev2, shadow2, rot2, 1000, 2, zord='pixel')
  par(mar=c(1,0,1,0))
...
elren <- mrender_elevation(
  volcano, shadow, list(rot.l, rot.r), res=600, d=200, persp.mode='abs'
)
left <- flip(elren[[1]])
right <- flip(elren[[2]])
```

We can then merge the two images to make an analygraph:
