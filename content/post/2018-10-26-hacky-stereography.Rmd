...
```{r}
## We start with a matrix of elevations
...
mx.l[,'x'] <- mx.l[,'x'] - diff(range(mx.l[,'x'])) / 2 - eye.sep / 2
...
## It is possible that after rotation order along the x-axis has changed due to
## height differences, so we need to get the cummmax per row.
...
x.grid <- findInterval(x.rng, mx.l.r[,'x'])
x.grid[x.grid == 0L] <- 1L

## Interpolate the shadows from the original values onto our gridded x values
## `pmin` to deal with grid values to the 'left' of leftmost defined x value

x.offset <- pmin((x - x.lo), 0) / (x.lo - x.hi)
shadow.interp <- x.offset * z.hi + (1 - x.offset) * z.lo

(x.hi - pmin(x, x.lo)) * (x.lo - x.hi) * z.lo

mx.l.r.g <- apply(m.l.r, 

In order to to remap our data into a grid, we're going to find what interval

## in the rotated data each our grid x-coordinates is in.

ggplot(as.data.frame(h.c.r), aes(x=x, y=y, color=z)) +
  geom_point() +
