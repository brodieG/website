...
`heightmap` is an elevation matrix.  The image we open the post is 550 x 505
pixels, so with 25 elevations and 100 long paths this requires up
...
The two orders of magnitude difference in timing is typical.
...
writeLines(c("<pre>", fansi::sgr_to_html(coords), "</pre>"))
```
...
R will never be as fast as compiled code, but you can get close in most cases
