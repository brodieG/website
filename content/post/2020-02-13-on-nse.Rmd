...
instead of just against objects in the workspace.  Compare to what happens with
a command that is evaluated in the standard way:
...
R prompt are kept (e.g. `w <- c("am", "I", "global")`).  Our substituted command
...
out of Enclosures.  We'll call this sequence of Enclosures the environment
chain.
...
chain and the `base::rev` function from the base package[^pathological-pkg].
...
lookup-and-substitute process on any R commands contained in the body of the
...
environment becomes the **Evaluation** Environment for the commands in the
function body:</span>
...
we reversed the command `rev(cmd2)`.  Here is what `with2_bad` effectively did:
...
It's mind boggling enough that we can reverse an unevaluated R command,
but imagine you're the user and everything was perfectly fine until the fateful
day you used the `cmd2` name in your command.  On the "Principle of Least
...
When evaluating with `with2_bad` as the enclosure `cmd2` resolves to the object
...
Since we can capture user commands, we can also edit them before we run
...
`mean_call` is injecting `mean` into the call template in the second line of
the function[^strained-metaphor]:
...
around with it for NSE is even more so.  This all compounded by the fact that
things that work fine in the workspace stop working correctly in functions, and
even then only in some circumstances.
