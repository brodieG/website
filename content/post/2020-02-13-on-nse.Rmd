...
changed by allowing the name `hp` to resolve against columns from `mtcars`
instead of against objects in the workspace.  Compare what happens with a
command that is evaluated in the standard way:
...
for fast lookups.  This name -&gt; object mappings are known as the "Frame" of
...
Functions though introduce some additional wrinkles.
...
statically compiled machine code, and contain no R commands or
environments[^static-compile-funs].</span>

<span id=fun-eval-env>Each time a closure is called a new environment is
created, enclosed by the **Function** Environment, and with the parameter ->
...
the one command `UseMethod("rev")` in the body.
...
the global environment becomes the "Calling Environment", so named because
it was the active Evaluation Environment when `rev` was **called** via the
command `rev(w)`.</span>
...
in `rev(w)` were matched against it.  `L` masked the name search as [`fun`'s
...
here) as the enclosure.  This is possible because environments are [akin to
named lists](#env-desc).
...
is no different in concept to what functions do, but because we are doing it by
interrupting evaluation and manually setting the Evaluation Environment it
...
What we want to quote is the command supplied as the argument `cmd`, not the
name `cmd`.  Thankfully R in its infinite flexibility provides a mechanism for
...
`quote` except it substitutes the unevaluated command passed as the argument
...
`enclos`.  This is unfortunately one of those areas where both R is not as clear
as it could be about the names of things, and there are closely related concepts
that need to be clearly distinguished[^par-vs-enc].
...
Environments, but the commands that we capture and wish to re-evaluate refer to
names in the [Calling Environment](#calling).  For things to work correctly we
must hand craft an environment chain that passes through the Calling
Environment.
...
we reversed the command `rev(cmd2)`:
...
It's mind boggling enough that we can actually reverse an unevaluated R command,
but imagine you're the user of a `with2_bad`-like function that's been working
perfectly fine until one fine day you use the `cmd2` name in your command.
In the "Principle of Least Surprise" scale that's right there with finding a rat
in your toilet bowl.

So what happened?  We had a name clash with the `cmd2` symbol that exists both
...
lookup-and-substitute process on `rev(cmd2)` (`<w2_b>` is `with2_bad`'s
...
`eval` so everything works as expected.  `with2`'s Evaluation Environment is out
...
Environment, and hence the Enclosure of its **Evaluation** Environments, is the
global environment.  So `friendly_fun`'s Evaluation Environment is not part of
...
NSE goes beyond to changing the environment chains used for evaluation.
Since we capture a user command, we could in theory edit it before we run it.
For example, suppose we wanted to write a function to sum things in the context
of a data frame, by group.  It could look something like:

```{r}
mean_call <- function(cmd) {
  mean_call <- quote(mean(NULL))
  mean_call[[2L]] <- cmd
  mean_call
}
mean_by_group <- function(data, cmd, group) {
  call_env <- parent.frame()
  group <- eval(substitute(group), data, enclos=call_env)
...
mean_by_group(iris, Sepal.Length / Sepal.Width, Species)
```

Don't worry about the details of how this is implemented.  The important point
is that we're calling `eval` with the `enclos=call_env`.  Previously this
ensured everything worked fine, and it seems to here, but what if:
...
Normally this is the type of problem that is obviated by the use of packages and
associated namespaces.  This ensures that when package functions evaluate they
resolve their names without interference from user objects.  But this doesn't
work here.  To understand why let's look at the modified command `mean_call`
produces:

<pre><code><span style='background-color: #55ff55;'>mean</span>(<span
style='background-color: #5555ff;'>Sepal.Length/Sepal.Width</span>)</code></pre>

The problem is that we need the green name `mean` to be resolved according to
our function's Evaluation Environment, but the blue command
`Sepal.Length/Sepal.Width` to be resolved according to the Calling Environment.
We can only evaluate a command in one environment, so this is not possible
directly.

One possibility is to use `base::mean` instead of `mean`, but this would still
fail if someone re-defined `::` in the Calling Environment (and yes, they can).
Another work-around is to manually substitute the actual function instead of the
name of the function:

```{r}
mean_call2 <- function(cmd) {
  mean_call <- quote(NULL(NULL))
  mean_call[[1L]] <- mean
  mean_call[[2L]] <- cmd
  mean_call
}
...
Now, provided that `mean_call2` is defined in a package we get:
...
This is ugly, but by pre-resolving the function to the object according to our
function's Evaluation Environment we avoid any conflicts with user names in the
Calling Environment.

But what if someone redefined `UseMethod` in the Calling Environment?  Well,
that is no problem as when the function is invoked, the environment chain will
be changed to be enclosed by that functions environment and `UseMethod` will
resolve as intended.

# Conclusions

Almost all the pain associated with using NSE in functions can be traced to
resuming evaluation in environment chains that mistakenly set the Enclosure
to the current Evaluation Environment instead of the Calling Environment.  This
is further compounded by the two chains often being similar, which may cause
inconsistent failures, and much hair pulling.
...
The [R documentation][31] uses the term
...
[^par-vs-enc]: Unfortunately the terminology around the hierarchical
  ...
  Calling Environment, i.e. the evaluation environment of the command that that
