...
# Logo stuff, very specific to this file
...
d[2:3] <- lapply(d[2:3], function(x) (x - min(x)) / diff(range(x)) * .9 + .05)

library(gridBezier)

brle <- rle(cmd == "C" | cmd == "")
ends <- with(brle, cumsum(lengths)[values])
starts <- with(brle, cumsum(lengths)[!values])
...
lrle <- rle(cmd == "L")
ends <- with(lrle, cumsum(lengths)[values])
starts <- with(lrle, cumsum(lengths)[!values])

lns <- Map(
  function(start, end) {
    points <- d[start:end, c('x', 'y')]
    grid.lines(points[[1]], points[[2]], gp=gpar(col='#000000'))
  },
  starts[seq_along(ends)],
  ends
)
 # par(mai=numeric(4))
 # plot.new()
 # lapply(bzs, grid.draw)
 # lapply(lns, grid.draw)
...
was the active Evaluation Environment when `rev` was **called** via the command
`rev(w)`.</span>
...
commands can be directly manipulated by R itself.  For now all we care about is
...
but imagine you're the user and everything was perfectly fine until one day you
happen to use the `cmd2` name in your command.  On the "Principle of Least
...
When evaluating with `with2_bad` as the enclosure `cmd2` resolves to the wrong
object.
...
`mean_call` takes an unevaluated R command of the type produced by `quote` or
...
`mean_by_grp` splits the data into groups and then evaluates the above command
...
calling `eval` with the `enclos=call_env`.  Previously this ensured everything
...
Calling Environment.  We can only evaluate a command in a single environment, so
we're stuck.
...
`mean_call` is injecting `mean` into the call template[^strained-metaphor]:
...
safely `eval` the command in a different environment chain:
...
writing the post.  Hopefully you found it instructive.  I'll consider it a
small victory if you managed to get this far.  Unfortunately even the standard
evaluation model in R is complex, so messing around with it for NSE is even more
so.
