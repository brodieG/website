...
col.vals <- sample(viridisLite::viridis(colors), colors * groups, rep=TRUE)
margin <- 0.1
rows <- 10
coords.raw <- c(margin, margin, 1 - margin, 1 - margin)
coords.scale <- coords.raw / rows
shift <- (1 - 2 * margin) / (rows - 1) * (0:(rows - 1))
...
rect(coords[,1], coords[,2], coords[,3], coords[,4],
  col=rep(viridisLite::viridis(25), 4))
...
2. Read offset in group  (int)*  Random seek in lvls long vector
3. Read input            (dbl)
4. Write to output       (dbl)*  Random seek in lvls long vec of vecs
5. Increment offset      (int)
5. Write offset          (int)   Should still be in cache
