...
times <- data.frame(
  ...
  time=c(14.184, 9.867 , 7.959, 3.697, 4.915, 2.652)
)
ggplot(times, aes(x=Version, y=time)) +
  geom_col() +
  facet_grid(.~Function, scales='free_x', space='free_x') +
...
The key advantage `group_slope2` has over the `rowsum` version is that we can
re-use the last-element-in-group index `gnc` since the groups are the same at
every step.  Once we know the last element in a group the sum using the
cumulative method is substantially faster:
...
The skeptical reader might have noticed that unlike with previous examples, we
