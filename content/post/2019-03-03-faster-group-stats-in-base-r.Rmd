...
that memory systems use to hide the horrendous latency of their RAM.  If you
already know about cache effects and the role they play in R, or if you are
...
We know that the performance improvement must come from `split` because the
result of that step is identical irrespective of whether the inputs are sorted
or not:
...
[`treeprof`][22] gives us detailed timings for the random and sorted `split`
calls[^treeprof-manip]: <span id=split-times></span>
...
highlighted numbers to start.  The columns they are in correspond to the "self"
time, or the time spent in the function as opposed to in child R-level
functions[^Rprof].  Usually these correspond to the evaluation of internal
compiled code as called via "special" functions[^special-internal].

For `split.default`, it is the [`.Internal` call that invokes][29] the [C
code][25] that allocates the result list of numeric vectors, and copies the
input values into them.  There is a ~15x speed-up between the sorted (137ms) and
randomized versions (1,997ms) for this step!
...
but this will take us much longer than in the sorted case.
...
average latency. Our model assumes that sequential accesses are met at L1
speeds.  In our (sorted) token/slot analogy the latency is the time it takes to
find a slot, and the bandwidth the rate at which we can transfer tokens into
their slots.

`split.default` has six memory accesses[^split-ops].  If the data fits in L1
cache they are all done at L1 speeds.  This explains why the chart starts at
24 cycles per element for both the random and sorted cases[^access-dominates].
In the token/slots analogy, a data set that fits in L1 is akin to a set of
distinct labels small enough that we can memorize the slot locations for each of
them.
...
random set, things slows down as an increasing number of the
non-sequential accesses must come from slower memory.  Each transition to a
slower memory is characterized by a steep processing time increase that
...
times in `split.default`](#split-times).  It performs remarkably well for
other working set sizes:
...
Individual timed runs are shown as points, and the median time for each working
set size shown as a diamond.

The decreasing times per item for the smaller sets are likely a result of R
function call overhead being amortized over an increasing number of entries, and
the jump up for the largest working set size likely corresponds to that size
defeating the normal latency-concealing optimizations used for sequential
accesses[^possible-failure].  Perhaps the equivalent in our token and slots
example is our hands cramping up after too many tokens.
...
model appears to approximate real timings well, it may be partly a coincidence.
...
There are many interesting things going on[^interesting-indexing], but the most
notable one is that in the random case we can keep up L2 speeds (~10 cycles) in
working sets 4-8 times L2 cache size, and that at 256 times beyond L3 sizes we
are still substantial faster than main memory latency.  Presumably the index
...
One fineal note: I clearly remember peeking at the [most popular QA on
...
[^possible-failure]: One point of failure could be that some level of the
  ...
  virtual to physical memory addresses is also exceeded and a full main memory
  table walk is required.
