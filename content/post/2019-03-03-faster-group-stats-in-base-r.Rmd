...
(x.cum.pre <- xc[bad.grp.start - 1])  # end val of prior group
```
```
...
```{r eval=FALSE}
(x.cum <- xc[bad.grp.end])             # end val of bad group
```
```
...
```{r eval=FALSE}
bad.grp.mean <- (x.cum - x.cum.pre) / length(bad.grp.idx)
rbind(x.cum.pre, x.cum, bad.grp.mean)
```
```{r echo=FALSE, results='asis'}
val <- "                                    [,1]
x.cum.pre    \033[42m3084340.36794573\033[43m348\033[m01554679871
x.cum        \033[42m3084341.21387870\033[43m702\033[m89254188538
...
```{r eval=FALSE}
(x_ux2.cum.pre <- cumsum(x_ux2)[bad.grp.start - 1])
```
```
...
```{r eval=FALSE}
x_ux2.diff <- x_ux2.cum - x_ux2.cum.pre
rbind(x_ux2.cum.pre, x_ux2.cum, x_ux2.diff)
```
```{r echo=FALSE, results='asis'}
val <- "                                                 [,1]
x_ux2.cum.pre \033[42m462824.40162014\033[43m582\033[m89287984371185302734
x_ux2.cum     \033[42m462824.40162014\033[43m611\033[m99671030044555664062
x_ux2.diff         0.000000000\033[43m29\033[m10383045673370361328"
writeFansi(val)
```
...
```{r eval=FALSE}
seven31::reveal(x_ux2.diff)
```
```{r echo=FALSE, results='asis'}
val <- "\033[31m0\033[m \033[34m01111011111\033[m  (-32) 0100000000000000000000000000000000000000000000000000 : x_ux2.diff"
writeFansi(val)
```
...
If we assume that the precision ends at the `1`, then we have 3 bits of
precision, which should translate to about &plusmn;6.25%.  The actual error is
-1.2%:
...
0 01111111110 (-1) 0101111000000100000110000000000000000000000000000000 : a - b
                  ...
                  101000000000000000000   # target #
                    ...
                    ^
                     111100000010000011   # smallest?
                         11111101111101   # sum(X_UX^2), old, possibly wrong
...
011001001001011111001111011011000000000   # x_ux2.cum.pre
                  ...
                  101000011111101111101   # sum(x_ux^2)
                     67894
                         0
# smallest value that we can add sum(X_UX^2) that causes 2^-33 bit flip.

0 01111011101 (-34) 1111100000010000011000000000000000000000000000000000 : a - b

                   1111110000001000001100000
                    ^
0110010010010111110011101101100000000000  # x_ux2.cum.pre
