...
My processor is clocked at 1.2GHz, so L1 cache can be accessed in 
`$4/1.2GHz = 2.5ns$`.  Each subsequent level of cache is slower, up
to main memory which is two orders of magnitude slower.  The flipside is that
...
access time by working set size.  We will assume that sequential reads can be
made at L1 cache speeds without polluting cache[^stream-buffer], and that other
accesses will be from the fastest available cache with hit rates proportional to
the ratio of cache size to working set size.  This is what the model looks like:
...
colnames <- unique(names(res.all))
res.all.mx <- matrix(res.all, ncol=4, dimnames=list(NULL, colnames), byrow=TRUE)
mx.lens <- vapply(res.all.mx[,1], nrow, 0)
resdf <- data.frame(
  random=unlist(lapply(res.all.mx[, 1], function(x) x[,3])),
  sorted=unlist(lapply(res.all.mx[, 2], function(x) x[,3])),
  n=rep(unlist(res.all.mx[, 3]), mx.lens),
  times=rep(unlist(res.all.mx[, 4]), mx.lens)
)
resdfm <- transform(
...
extra <- list(
  ...
  scale_shape_manual(values=c(Single=21, Median=23)),
...
working set size.  This is why the sorted case is modeled at a consistent 24
cycles per element, or four cycles for each of the six required memory accesses.
As the working set grows past each level of cache, an increasing portion of the
accesses will be from the slower level.
...
component double vectors, and an integer vector that tracks the offset within
each group that the next element needs to be written to.  This integer vector is
much smaller as only one value per group is needed, and integers are half the
size of doubles.  With ten elements per group this vector exceeds 4MB, the size
of the L3 cache, when there are ~10 million elements.
...
#   shape='Single'
# )
resmm <- tapply(resdfm$time.per, resdfm[c('variable', 'n')], median)
resdfmm <- melt(resmm)
...
All axes are log base 2.  The dashed vertical line represents our current
working set size (i.e. 10MM entries)[^working-set-size].

Whether the data is ordered does not appear to affect performance so long as it
fits in completely in L1 cache.  In that case, the CPU is able to quickly find
the target vector for each element in the input.  The declining timings from set
size `$2^10$` through `$2^15$` are likely due to amortization of fixed overhead
in the `split.default` call.  It is difficult to be certain that all operations
for data sets at or below `$2^15$` is indeed being fed out of L1 cache, but
given the timing per element is ~24 cycles and there are six memory operations
involved for each element copy, it is highly probably this is
all indeed happening in L1[^time-in-cycles][^split-ops].

```{r echo=FALSE}

mdl.dat <- data.frame(
  x=log2(set.sizes), y=log2((times + 4 * 4) / 1.2e9),
  variable='modeled'
)
resdfm2 <- transform(subset(resdfm, variable == 'random'), variable='observed')
resdfmm2 <- transform(subset(resdfmm, variable == 'random'), variable='observed')
ggplot(
  resdfm2,
  aes(x=log2(n * (8 + 4/10)), y=log2(time.per), color=variable, shape=shape)
) +
  extra +
  geom_text(
    data=subset(dat.label, facet == 'time'),
    aes(
      x=size + .125, y=values, label=label, color=NULL, group=NULL, shape=NULL
    ),
    color='black', hjust=0
  ) +
  geom_point(data=resdfmm2, size=3) +
  geom_point(
    data=mdl.dat,
    aes(x=x, y=y, shape=NULL), size=1
  ) +
  geom_vline(data=NULL, xintercept=cache.sizes, linetype=cache.styles) +
  scale_color_manual(values=c(observed='#cccc00', modeled='gray35')) +
  ggtitle("Random Order Timings, Observed and Modeled")
```

It would be as if we were only given 1-4 different token
labels in the previous game: we would memorize the slot locations and transfer
the tokens quickly.

This is the computer equivalent of dealing
with the tokens bearing a small set of the possible labels, say 1-4, so that we
can memorize the corresponding slot locations and transfer tokens with little
thought.

two, three, or maybe four label/slot combinations in the token/slot game
from earlier.  We can easily remember the slot locations  
them 


is a bit of noise int 


* Computes the size of each group by tabulating factor levels.
* Allocates a list of numeric vectors, one for each group of size as per above.
* Scans through the input and the groups in step, copying the input values to
  the vector corresponding to its group.

These are three in essence three loops that have no order dependency in them, so
in theory they should execute in the same amount of time.  While the logic of
the code is not affected,

a calls to `.Internal(split(x, f))`,
where `x` is the vector to split and `f` the factor to split along.  That code
essentially boils down to:

* Loop through factor and compute how many instances of each level there are
* Allocate list of numeric vectors, each of size computed in prior step
* Scan through `x` and `f`, copying values of `x` to the numeric vectors in the
  previously allocated list as dictated by `f`.
