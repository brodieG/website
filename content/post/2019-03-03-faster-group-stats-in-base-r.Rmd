...
slope2a <- function(x, y, grp, splits=5) {
  ...
  cuts <- as.integer(round(seq(1L, length(grp.len), length.out=splits)))
  cuts[length(cuts)] <- cuts[length(cuts)] + 1L

  grp.len.o.c <- cumsum(c(1L, grp.len.o))
  res <- vector("list", splits)

  for(i in seq_len(splits - 1L)){
...
x2 <- runif(100)
y2 <- runif(100)
g2 <- sample(1:10, 100, rep=TRUE)
zz1 <- slope2(x2, y2, g2)
zz2 <- slope2a(x2, y2, g2)
