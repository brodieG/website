...
Based on this observation I propose a new function for base:

```{r eval=FALSE}
rowsums <- function(x, group) {
  o <- order(group)
  rowsum(x[o], group[o], reorder=FALSE)
}
```

The pluralization in combination with the lower case will document that this is
the pre-sorting version of `rowsum`[^love-r-but].

One last thing before we move on: in theory it should be possible to
re-implement the `unique` and `rowsum` internals to assume sorted inputs/groups.

```{r eval=FALSE}
unique2 <- inline::cfunction(sig=c(x='integer'), body="
  R_xlen_t len, i, j = 0, len_u = 1;
  SEXP res;
  int *xi = INTEGER(x);
  len = XLENGTH(x);
...
rowsums <- inline::cfunction(
  ...
  if(len > 1) {
    ...
    int *res_gi = INTEGER(res_x);
    memset(res_xi, 0, len_u * (sizeof *res_xi));
    R_xlen_t j = 0;
  ...
  return res;
")
copy <- inline::cfunction(sig=c(x='integer'), body="return x;")
```
