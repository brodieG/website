...
but that is not actually available outside of `cumsum`.
...
and got a value very close to the actual error:
...
showed when we compared the [80 and 64 bit versions of `A.n`](#a-err).  We got
the 80 bit version of `A.n` by adding the error to the 64 bit version[^by-hand].
