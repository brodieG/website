...
group_slope <- function(x, y, grp) {
  ...
  gn <- rowsum(rep(1L, length(xo)), go)
  gi <- rep(seq_along(gn), gn)

  ## compute mean(x) and mean(y), and recycle them
  ## to each element of `x` and `y`:
  xs <- rowsum(xo, go)     # sum(x)
  xu <- (xs/gn)[gi]        # mean(x), recycled to original vec length
  ys <- rowsum(yo, go)
  yu <- (ys/gn)[gi]
...
```{r eval=FALSE}
gn <- rowsum(rep(1L, length(go)), go)
gi <- rep(seq_along(gn), gn)
```

By summing ones (`rep(1L, length(go))`) by group we count how many elements
there are in each group and store than in `gn`.  We can illustrate in simplified
form:

```{r}
(xo <- 2:6)                              # some values
(go <- c(3, 3, 5, 5, 5))                 # their groups
(gn <- rowsum(rep(1L, length(go)), go))  # the size of the groups
```
...
```{r}
(xs <- rowsum(xo, go))                   # sum of each group
(xu <- xs / gn)                          # mean of each group
```
...
times <- data.frame(
  ...
  time=c(14.184, 9.867 , 7.959, 3.697, 5.139)
)
...
So like Rocky Balboa we walk off into the sunset, not victorious, but satisfied
we put up a good fight.  Except...

# Round II

I've had the idea that we could use cumulative functions to compute group
statistics for a while, but [John Mount][10] [beat me to it][11].  I may have
seen this elsewhere previously and forgot the seeing but not the concept, so
apologies if I'm subconsciously stealing someone else's idea.  Some quick
research didn't turn up anything.  Also, remember how we saw earlier that a fast
order opened up opportunities for [new algorithms](#alt-algo)?  Well, this
cumulative approach was not really practical until `data.table` radix sorting
was added to base R in version 3.3.0.
...
the help of `base::rle`.  `rle` returns the lengths of runs of adjacent numbers
that are equal.  We can use it to compute the lengths of each group:
...
```{r}
xc.last <- xc[cumsum(rle(go)[['lengths']])]
```

To finish we just take the differences.  We can wrap the whole thing into a
function[^group-sum-complete]:
...
We return some extra information so that we can use it in future steps if
needed.

Every step of `group_sum` is internally vectorized, so the function is fast.  We
demonstrate here with the original 10MM data set:
...
This is neck and neck with the [pre-ordered `rowsum`](#rowsum-ordered)
calculation, but importantly it allows us to recover intermediate calculations
we can re-use.
...
Original `*pply`, we used in the [prior grouped statistics post][8]:
...
group_sum2 <- function(x, grp, na.rm=FALSE) {
  ...
  if(!na.rm && has.na)
    x.grp.c[findInterval(na.x, grp.rle.c, left.open=TRUE)] <- NA
  if(any.inf) {
    inf.grps <- findInterval(inf.x, grp.rle.c, left.open=TRUE)
    inf.rle <- rle(inf.grps)
    inf.vals <- rep(Inf, length(inf.grps))
    inf.neg <- inf.x < 0
    ...
    if(any(inf.long <- (inf.rle[['lengths']] > 1L))) {
      ...
      inf.vals[inf.neg.g] <- -Inf
      inf.vals[inf.pos.g & inf.neg.g] <- NaN
    } else {
      inf.vals[inf.neg] <- -Inf
    }
    x.grp.c[inf.grps] <- inf.vals
  }
  structure(x.grp.c, groups=grp.rle[['values']], n=grp.rle[['lengths']])
}
system.time(group_sum2(x, grp))

RNGversion("3.5.2"); set.seed(42)
x2 <- c(1:10)
x2[c(2, 6)] <- Inf
g2 <- sample(1:3, 10, rep=T)
group_sum2(x2, g2)
x2[c(2, 6)] <- NA
group_sum2(x2, g2)
group_sum2(x2, g2, na.rm=TRUE)
...
[prior post][8]
[^slope-mode]: This is a slightly modified version of the original from [prior
