...
system.time({
  ...
  x.ord.c <- cumsum(x.ord)
  x.grp.c <- cumsum(x.ord)[grp.rle.c]
  x.grp.c[-1L] <- x.grp.c[-1L] - x.grp.c[-length(x.grp.c)]

  y.grp. <- cumsum(y.ord)[grp.rle.c]
  y.grp.c[-1L] <- y.grp.c[-1L] - y.grp.c[-length(y.grp.c)]
  ...
  x_ux.y_uy <- cumsum(x_ux * y_uy)[grp.rle.c]
  x_ux.y_uy[-1L] <- x_ux.y_uy[-1L] - x_ux.y_uy[-length(x_ux.y_uy)]
  x_ux2 <- cumsum(x_ux ^ 2)[grp.rle.c]
  x_ux2[-1L] <- x_ux2[-1L] - x_ux2[-length(x_ux2)]
