...
My processor is clocked at 1.2GHz, so L1 cache can be accessed in `$3 \times
1/1.2GHz = 2.5ns$`.  Each subsequent level of cache is slower, up
...
One way we can inspect real life effect of cache effects is to vary the size of
our problem and time each run:
...
resdf <- transform(resdf, ratio=random/sorted)
...
extra <- list(
  ...
  ylab("Time Per Input Element, Seconds, Log Scale"),
  xlab("Result Size in Bytes, Log Scale"),
...
ggplot(
  ...
  geom_vline(data=NULL, xintercept=cache.sizes, linetype=rep(cache.styles, 2)) +
  scale_color_manual(
    values=c(random='#cccc00', sorted='#33ee33', ratio='#3333ee')
  ) +
...
set.sizes <- setNames(2^test.sizes * (8 + 4/10), test.sizes)
hit.rates <- outer(
...
mdl.dat <- data.frame(
  x=log2(set.sizes), y=log2((rowSums(times) + 20) / 1.2e9),
  variable='modeled'
)
