...
sum.  But when values are so close, doing so is the computing equivalent of
taking the steps down into a dark basement with ominous background music in a
...
corresponds to the implicit value bit that isn't shown, and `2^-2` to the one
`1` at the second position in the "normalized value" portion of the double.
Each bit in the "normalized value" section is represented by `$2^{-n}$` where
`n` is the position in the bit field.  All those digits are just an artifact of
the conversion of a limited precision binary number into decimal representation.

So how much precision do we actually have?  The naive decimal view suggests not
that much.  The binary view suggests about 3 bits worth, but we don't actually
know if any of the trailing zeroes are significant.  The actual error is -1.2%,
and that suggests closer to 4-5 bits of precision.  To get to the actual answer
we need to dive deep into the extended precision binary representation used
`cumsum`.  Here it is for the last value of the previous group, compared to the
...
And for the last value of the current group:
...
luck the difference between the 80 bit values and 64 bit values is small, and
also in the same direction.  As a result the total error caused by the rounding
from 80 bits to 64 bits is `$(000111100 - 101111011) \times 2^{-38}$` (forgive
the mixing of binary and decimal representation).  We can compute this:

```{r}
sum(2^-(38:45) * (c(0,0,1,1,1,1,0,0) - c(1,0,1,1,1,0,1,1)))
```

Recall that binary representation can be translated into `$2^{-n}$`.  
That's exactly what we're doing here.  We take the powers of two between
`$2^-38$`
