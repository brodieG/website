...
as fast as in the [unsorted case](#sum-unsorted), **including the time required
to sort the data**.  We just doubled the speed of an important workhorse R
function without touching the code.
...
with `data.table`, but what's the fun in being gracious nowadays?  If that
satisfies your curiosity feel free to skip to the [next
section](#so-you-think-you-can-group-stat).

We know that the performance improvement must come from the `split` step since
it orders the groups, so the `vapply` step in both the sorted and unsorted input
cases is the same:
...
Let's look at the [`treeprof`][22] timings for the two `split` calls:
<span id=split-times></span>
...
For each of the unordered and ordered versions, `treeprof` reports the time
spent evaluating each function[^treeprof-manip], and the amount of that time
that cannot be attributed to child R-level function calls[^Rprof].  For example
in the unordered case the `as.factor` call is evaluated in 5,242 milliseconds,
of which 2,771 (5,245 - 2,471) are spent in child R-level function calls.  The
balance, 2,471 milliseconds, is spent on evaluations outside of the R call
stack, typically internal compiled code as called via "special"
functions[^special-internal].

Indeed splitting is much faster when the data is ordered.  The primary sources
of the difference are:

* "special" code in `split.default`:  1997 - 137 = 1860ms (~15x speed-up)
* "special" code in `as.factor`:      2471 - 630 = 1841ms ( ~4x speed-up)
* "special" code in `unique.default`: 2705 - 789 = 1906ms ( ~4x speed-up)

For `split.default`, the "special" code is the [internal C code][25] that
allocates the result list of numeric vectors, and copies the input values into
each list element according to the groups.  To understand the performance
differences here we can play a game that approximates what the code does.

Imagine we have been given a board full of tokens, each identified by one of
eighteen possible two letter combinations[^game-size].  We have also been given
a box with eighteen slots, each one labeled with one of the possible letter
combinations.  Our job is to transfer the tokens from the board into the slot
with the same label:

```{r echo=FALSE, eval=FALSE}
strings <- sample(do.call(paste0, expand.grid(LETTERS, LETTERS)), 18)
strings.s <- matrix(sample(rep(strings, 8)), ncol=8)
for(i in seq_len(nrow(strings.s))) cat(strings.s[i,], '\n')
```
```
        TOKENS                        SLOTS
        ------                        -----
CE DM FK LB PL QU UE VF               QG [ ]
CE DM FK LB PL QU UE VF               XD [ ]
CE DM FK LD PL QU UE XD               DM [ ]
CE DM FK LD PL QU UE XD               PL [ ]
CE DM JY LD PL QU VA XD               CE [ ]
CE DM JY LD PL QU VA XD               RF [ ]
CE DS JY LD PL RF VA XD               ZB [ ]
CE DS JY LD PL RF VA XD               QU [ ]
CK DS JY LD QG RF VA XD               VA [ ]
CK DS JY LD QG RF VA XD               VF [ ]
CK DS JY PH QG RF VA ZB               UE [ ]
CK DS JY PH QG RF VA ZB               LD [ ]
CK DS LB PH QG RF VF ZB               DS [ ]
CK DS LB PH QG RF VF ZB               LB [ ]
CK FK LB PH QG UE VF ZB               FK [ ]
CK FK LB PH QG UE VF ZB               JY [ ]
DM FK LB PH QU UE VF ZB               CK [ ]
DM FK LB PH QU UE VF ZB               PH [ ]
```

We choose to start with the first "CE" token.  We pick it up and scan the slot
labels for the matching slot until we find it and drop the token in it. The next
token down is also labeled "CE".  This time, since we remember where we just
dropped the last token we can repeat the action with little fuss.  This allows
us to quickly transfer the remaining sequence of six "CE" tokens into their
slot.  Then we must slow down to find the "CK" slot, but again, after the first
transfer the seven subsequent ones can happen quickly because we remember what
slot we need to go back to.

Even though we have 144 tokens we can do the transfer reasonably quickly because
we only need to do eighteen full slot scans.
...
```
        TOKENS                        SLOTS
        ------                        -----
PH VA FK PH ZB JY DM XD               QG [ ]
LB QU VF LB QU JY XD DS               XD [ ]
QG QG JY LD VA QG LD RF               DM [ ]
DM RF QU XD RF XD CK PH               PL [ ]
VA FK XD DS UE JY JY PL               CE [ ]
QG UE UE CE VF PL DS XD               RF [ ]
FK LD PH CK UE PH LD VF               ZB [ ]
QG CE FK LB LD ZB DM CE               QU [ ]
VF VA RF FK DM CK DM VA               VA [ ]
UE CE LB UE LB RF LB QG               VF [ ]
DM ZB QU VF ZB QU LB DM               UE [ ]
ZB DS QU JY JY PL PL QU               LD [ ]
ZB QU RF DS PH XD LD XD               DS [ ]
FK RF VA VF VA JY VF PL               LB [ ]
ZB CK CE CE CK PH CE CK               FK [ ]
PH FK FK UE VA VF DS LD               JY [ ]
ZB LD UE QG DS DM PL LB               CK [ ]
CK PL DS CE RF PL QG CK               PH [ ]
```

After our first "PH" token we have two choices: start with the next token with a
different label and scan the slot list again, or scan all tokens to find all the
"PH" tokens.  If we have a good memory we might be able to remember the location
for a few of the labels so that each time we hit one of them we can transfer to
the matching slot without needing to scan the slots.  Either way, it will take
us much longer than when the tokens were sorted, and this is essentially because
we can't easily memorize the locations of the eighteen slots.

Even though computers are much better at remembering than us humans, they resort
to some compromises to optimize the cost vs performance trade-off, and those
come into play here.  In particular, really fast memory is expensive so
computers only have a little bit of it and it is used as a temporary holding
pool for data and instructions while they are in use.  This type of memory is
known as "cache".

In addition to the main memory, my system has three levels of cache: L1, L2, and
L3.  Here are typical access times and sizes of various cache levels and:

<!--
<div style='max-width: 90%; white-space: nowrap; margin: auto;'>
...
library(ggplot2)
ggplot(data.frame(labels=labels, cycles=times)) +
  geom_col(aes(labels, cycles))
ggplot(data.frame(labels=labels, sizes=sizes)) +
  geom_col(aes(labels, sizes)) + coord_cartesian(ylim=c(0,2^24))
```
</div>
</div>
-->
...
My processor is clocked at 1.2GHz, so L1 cache can be accessed in ~2.5
nanoseconds (`$4/1.2GHz = 2.5ns$`).  Each subsequent level of cache is slower,
up to main memory which is two orders of magnitude slower.  The flipside is that
there is very little L1 cache[^l1-data-vs-inst], a measly 32KB.  So little that
if I didn't truncate the plot at 300MB you wouldn't be able to see it, and
certainly nowhere near large enough to hold 10 million double precision numbers.

Given the cache structure of my system we can we can build a model of memory
access time by working set size:
...
clrs <- c(random='#cccc00', sorted='#33ee33', ratio='#3333ee')
extra <- list(
...
The dashed vertical line represents our current working set size (i.e. 10MM
entries)[^working-set-size].

We assume that all sequential accesses can be can be efficiently
prefetched/buffered so that the main memory latency is concealed irrespective of
working set size.  This is akin to a board with tokens sorted in the same order
that the slots are in.  We can mechanically transfer the tokens from board to
slots with almost no scanning for the correct slot.  `split.default` has
six memory accesses, and if each is done at the fastest possible 4 cycles per
element, then each element can be processed in 24 cycles[^access-dominates].  

In the random case we model access to be the same speed as in the sorted case so
long as the working set fits in L1 cache.  This is akin to having only two or
three distinct labels in randomly ordered tokens.  We can easily memorize the
three slot locations and transfer the tokens quickly without further thought.
As the working set grows past each level of cache, an increasing portion of the
accesses will be from the slower next slower level.

One seeming anomaly is that there are only three transitions from faster to
slower memory, yet there are four bumps in the plot.  This is because our
working set contains two randomly accessed elements: the result list with the
double precision vectors that correspond to each group, and an integer vector
that tracks the offsets within each group that the next element will be written
to.  The last bump corresponds to the point at which the offset integer vector
outgrows L3 cache[^offset-vector].

This simple model predicts the random inputs will be ~13 times slower to process
than the sorted ones for 10MM long inputs.  This is close to the [previously
observed ~15 times in `split.default`](#split-times).  But what about different
sized inputs?   The model performs remarkably well:
...
Here we add as points actual timings at various working set sizes.  The median
time for each working set size is shown as a diamond.

The decreasing times per item for the smaller sets are likely a result of
overhead being amortized over an increasing number of entries.  Another
noticeable mismatch is at the largest working set size for the sorted case.
Here the tricks that the memory system can use to conceal latency in sequential
accesses start to fail[^possible-failure].  Perhaps the equivalent in our token
and slots example is our hands cramping up after too many tokens.
...
model appears to approximate real timings well, it could be a coincidence.
...
2. Performance can vary greatly with work load type.
    * Some use cases will be optimized.
    * Sequential accesses are likely to be highly optimized.
    * Random accesses will almost always be pathological.

As seen in the tests on our system these factors interact, e.g. all access types
were fast so long as the working sets fit in L1 cache, and the penalty for
pathological access balloons as the working set size grows.

While this explains why `split` works so much faster with sorted inputs, we are
still left to deal with the unsorted inputs.  Even ignoring the `order` step
itself, the indexing step required to order our data require random accesses:
...
Here we show the timings for `x[o]` as well as for a version with `x` and `o`
pre-sorted:

```{r echo=FALSE, fig.height=5}
resdfm <- proc_times(readRDS(sprintf(base, '-4')))
ymax <- max(resdfm$time.per)
dat.label <- as.data.frame(
  c(
    values=lapply(log2(ymax), rep, each=length(cache.sizes)),
    list(size=unname(cache.sizes)),
...
That the baseline times are faster than for `split.default` is expected given
there are fewer steps involved in indexing than in splitting. The sorted
indexing case is interesting, but I won't delve into it as ultimately it is not
relevant here[^sorted-indexing].  What is most notable is that in the random
case we can keep up L2 latencies (~10 cycles) up to 4-8 times L2 cache size, and
that beyond L3 sizes we are still substantial faster than main memory latency.
Presumably the access pattern is simple and common enough (read offsets from
indexing vector, fetch values using offsets) that the memory system can prefetch
and hide a good amount of the latency associated with the random accesses.

This example further highlights that there is a lot going on in the memory
system beyond what can be modeled from the basic cache size and latency.
Additionally, it suggests that there can be a substantial benefit to isolating
pathological (e.g. random) accesses to the simplest problems so that the memory
system will have the best chance to mitigate latency costs.

One final note: YMMV.  It is conceivable that some system are not able to
...
[^special-internal]: Compiled code functions that can be invoked with the
  `.Internal` or `.Primitive` interfaces without evaluating the parameters are
  considered "special".  Not all functions invoked with `.Primitive` or
  `.Internal` are special.  The `.Internal` function itself is a "special"
  primitive.  See [R Internals][23] for details.
[^Rprof]: `treeprof` collects its timings with `Rprof`, so you can consult
  `?Rprof` for more details about the "total" and "self" times which are the
  ones reported here.
...
[^time-in-cycles]: `$2^{-25.6}s per element / (1 / 1.2GHz) \approx 24 cycles per element$`.
[^split-ops]: The operations involving memory are: read input value (1), read
  factor level (2), fetch target vector from result list (3), read offset for
  target vector (4), write value to target vector at offset (5), and increment
  offset (6).
...
[^offset-vector]: With ten elements pre group on average, we will need ~1
  million elements in the offset vector to track each group with a 10MM
  working set size.  At 4 bytes an element, that makes for a ~4MB integer
  vector, which is the size of L3 cache.
[^possible-failure]: One point of failure could be that some level of the
  [Translation Lookaside Buffer][29] that acts as cache for the translation of
  virtual to physical memory is also exceeded and a full main memory table walk
  is required.
