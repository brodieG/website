...
each list element according to the groups.
...
colors <- 10
groups <- 10
# col.vals <- rep(sample(colors(), colors), groups)

rect_coords <- function(rows, margin) {
  coords.raw <- c(margin, margin, 1 - margin, 1 - margin)
  coords.scale <- coords.raw / rows

  shift <- (1) / (rows) * (0:(rows - 1))
  shifts <- expand.grid(xs=shift, ys=shift)
  coords <- matrix(coords.scale, nrow=100, ncol=4, byrow=TRUE)
  coords[, c(1,3)] <- coords[, c(1,3)] + shifts[[1]]
  coords[, c(2,4)] <- coords[, c(2,4)] + shifts[[2]]
  coords
}
col.vals <- rainbow(colors)[sample(colors * groups) %% colors + 1]
coords <- rect_coords(10, 0.1)

old.par <- par(bg='#CCCCCC', mai=numeric(4), mfrow=c(1,2))
plot.window(c(0,1), c(0,1), asp=1)
plot.new()
rect(coords[,1], coords[,2], coords[,3], coords[,4],
  col=col.vals, border=NA
)
coords2 <- rect_coords(10, 0)
plot.new()
rect(coords2[,1], coords2[,2], coords2[,3], coords2[,4],
  col=matrix(sort(col.vals), nrow=10, byrow=TRUE), border=NA
)
coords1 <- rect_coords(10, 0.2)
rect(coords1[,1], coords1[,2], coords1[,3], coords1[,4],
...
rds <- '../../static/data/group-stat-base-split-times.RDS'
res <- readRDS(rds)
...
plot.els <- list(
  ...
  ylab("Time Per Input Element, Log Scale"),
