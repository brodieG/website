...
Let's try a warm up round by computing sums of `x` grouped by `grp`.  
...
team contributed their radix sort to R:
...
In particular with `order`, the radix sort is **fifty** times faster than the
shell sort it replaces.

Reason #1 why this is a huge deal, as also shown in the tweet:
...
as fast, **including the time required to sort the data**.  We just doubled the
speed of an important workhorse R function without touching the code.
...
satisfies your curiosity feel free to skip to the next section.
...
* "special" code in `unique.default`: 2705 - 789 = 1906ms (3.5x speed-up)

We can tell the difference are in compiled code because 
...
ggplot(dat, aes(x=log2(n), y=log2(value), color=variable)) +
  ...
  geom_text(
    data=dat.label,
    aes(x=size, y=values, label=label, color=NULL),
    hjust=0
  ) + plot.els
...
2. Read offset in group  (int)*
3. Read source value sequentially - this need not be cached
