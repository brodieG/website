...
shoulders above all challengers. Yet, here we are, about to throw our had in
the ring with nothing but base R functionality.  Are we out of our minds?  We
don't have access C-level group statistics, nor efficient memory re-using
groups, but TL;DR, the results might surprise you.

Stick around for an epic battle between the champ and the scrappy challenger.
To go along with the main card: cameos from cache effects and branch prediction,
and a death match with the big bad villain IEEE-754.
...
Let's try a warm up round.  `sys.time` is the expression through `System.time`
eleven times and returns the median timing.  It is [defined in the
appendix](#sys.time).

```{r eval=FALSE}
sys.time(x.ref <- tapply(x, grp, sum))
```
```
   user  system elapsed
  7.137   0.163   7.308
...
Ouch.  Even without multithreading `data.table` just crushed the venerable but
outmatched `tapply`.
