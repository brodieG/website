...
digits[^precision-digits].  Normally this should still be plenty of precision,
but the next step `$x - \bar{x}$` peels off even more, as shown here with the
first value of `$x$` in the group:
...
```{r eval=FALSE}
diff <- x_ux2.cum - x_ux2.cum.prev
rbind(x_ux2.cum.prev, x_ux2.cum, diff)
```
```{r echo=FALSE, results='asis'}
val <- "                                                 [,1]
...
diff                0.0000000003492459654808044433594"
writeFansi(val)
```

Zilch precision left.  I will not transcribe the scream of horror and agony for
the sake of the children, but I will show you the aftermath:
...
* Normalized value (52 bits: <code>100000000000000...</code>), which is  the
  part that matters for precision.

If this does not knot up your guts I don't know what will.  We're essentially
left with two bits of precision: the only `1` visible in the "Normalized Value"
section, and implicitly, the leading `1` of that same value that is not shown.
...
This is exactly what we got from subtracting the cumulative values earlier, and
shows there really is very little precision.  The `2^0` corresponds to the
implicit value bit that isn't shown, and `2^-1` to the one `1` in the
"normalized value" portion of the double.  Each bit in the "normalized value"
section is represented by `2^-n` where `n` is the position in the bit field.  If
we take the two bits of precision at face value, we can represent four
normalized values, so a 15% error is about what you might expect.  In reality we
don't even have two bits of precision, but I thought it was striking enough to
see the binary representation of the value we are dealing with.

I hope you will forgive some of the hand-waving I indulged in above.  For
example, trailing zeroes don't necessarily indicate imprecision.  In fact, the
bit representation of the actual values we're subtracting would imply higher
precision than just two bits.  At the same time, the bit "precision" can greatly
overstate the actual precision of a number.  For example, if I add two values
that are each precise to two bits, but are of different magnitudes, the result
will appear as having much higher precision than it actually does.  The net of
all this is the above explanation should be mostly correct in spirit if perhaps
a bit liberal with the interpretation of some facts.

# Now What?

Before we give up hope it is worth pointing out that precision loss in some
cases can be considered a feature.  For example, [Drew Schmidt's `float`][17]
package implements single precision numerics for R as an explicit trade-off of
precision for memory and speed.  In a sense, we are doing something similar by
using `cumsum`, although not explicitly.

Additionally, in our case, the bad errors happen when all the `$x$` values in a
group are very close to each other, which produces very steep slopes.  Do we
actually care whether the slope is -2977:1 or -2512:1?  Maybe you shouldn't
compute injection orbits for expensive mars landers using these methods, but
otherwise it is conceivable the loss of precision might be acceptable.  But you
also need to have some awareness of the magnitude differences between the
overall sum of your data and any given group's sum.  If precision in the small
sums is important you cannot use this method as it is.

Either way, it would be nice if we could make a version of this method that
...
much larger than any given individual group in size.  This is what happened with
the [cumulative sum of `$(x - \bar{x})^2$`](#carnage), exacerbated by the
squaring of the variable.

If we could ensure the cumulative sum stays close to zero, then the error should
be greatly mitigated.  This is hard to do without knowing ahead of time what the
group sizes are.  Instead, we can use a two pass process where we first
compute the imprecise group sums, and then re-insert the negative of those
