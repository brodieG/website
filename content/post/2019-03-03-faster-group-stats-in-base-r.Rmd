...
labels <- c('L1', 'L2', 'L3', 'Main')
accews.times <- c(3, 15, 40, 250)
sizes <- c(2^15, 2^18, 2^21, 2^33)

library(ggplot2)
ggplot(data.frame(labels=labels, cycles=acccess.times), aes(labels, cycles)) +
  geom_col() + xlab(NULL) +
  geom_label(aes(label=acccess.times), vjust=-.25) +
...
res1 <- readRDS('static/data/group-stat-base-split-times.RDS')
res2 <- readRDS('static/data/group-stat-base-split-times-2.RDS')
res3 <- readRDS('static/data/group-stat-base-split-times-3.RDS')
...
cache.lab <- c('L1d\n32KB', 'L2\n256KB', 'L3\n4MB', '10MM\nItems')
cache.styles <- rep(c(rep('solid', 3), 'dashed'), 2)
dat.label <- as.data.frame(
  c(
    lapply(ymax, rep, each=length(cache.sizes)),
    list(size=cache.sizes), list(label=cache.lab), list(style=cache.styles)
) )
...
[^split-ops]: The operations involving memory are: read input value (1), read
  factor level (2), fetch target vector from result list (3), read offset in
