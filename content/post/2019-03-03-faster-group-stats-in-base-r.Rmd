...
system.time(slope.gs3 <- group_slope2(x, y, grp, group_sum=.group_sum_int_16))
```
```
   user  system elapsed
  2.311   0.703   3.026
...
```
       0%       25%       50%       75%      100% 
-1.80e-08 -5.50e-15  0.00e+00  5.47e-15  2.54e-06 
```

Indeed the errors are no larger than that.  Note the precision check occurs only
at the group sum step.  If other steps accumulate errors it is possible for the
final precision to end up below the specified levels.
