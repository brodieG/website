...
dropped.  So much so that nowadays it may take ~200 CPU cycles to have a CPU
data request fulfilled from main memory.  Memory bandwidth on the other hand has
kept up.  The over-used analogy is that of a freight train: it is slow to start
moving and get to you (latency), but if you can have it properly loaded it will
bury you in goods once it arrives (bandwidth).  If you can figure out how to
fully pack the freight train before it departs the cost of latency is low as you
suffer it once and amortize it over the entire train load.  That unfortunately
is an ideal case as often what you need will depend on ongoing calculations .


Ideally CPUs would request and get data with each of their cycles so that 
no processing power is wasted, but unfortunately this is not possible.  CPU
processing speed has grown faster than RAM access latency has dropped, and
