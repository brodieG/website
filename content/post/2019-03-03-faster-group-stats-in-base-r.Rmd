...
a representation][13] that allows [18-19 digits of precision][14] for
`cumsum`[^extended-precision].  Yet that is not enough here.  To see why we need
...
(x.cum.pre <- sum(xo[1:(bad.grp.start-1)]))  # end val of prior group
```
```
...
rbind(x.cum.pre, x.cum, group.sum)
```
```{r echo=FALSE, results='asis'}
val <- "                                    [,1]
x.cum.pre \033[42m3084340.3679457\033[m334801554679871
x.cum     \033[42m3084341.2138787\033[m070289254188538
group.sum       0.\033[42m8459329\033[m735487699508667"
writeFansi(val)
```
...
```{r echo=FALSE, results='asis'}
val <- "                              [,1]
x1    0.\033[42m422978624399751\033[m42390125029
ux    0.\033[42m422966486\033[m77438497543334961
x1_ux 0.0000\033[42m12137\033[m62536644846790068"
writeFansi(val)
```

Any time we subtract two numbers that are very similar we lose precision as in
this example and the prior one.  Additionally, as also shown in this example, we
will lose precision anytime we add or subtract numbers of mismatched
precisions[^prec-mismatch].
...
```{r echo=FALSE, results='asis'}
val <- "                                                 [,1]
x_ux2.cum.pre \033[42m46282\033[m4.4016201458289287984371185302734
x_ux2.cum     \033[42m46282\033[m4.4016201461199671030044555664062
x_ux2.diff         0.0000000002910383045673370361328"
writeFansi(val)
```

Zilch precision left[^mag-prec].  What about all those digits displayed?  They
...
decimal representation.  That there is seemingly even some precision left at the
