...
research didn't turn up anything, and it may be because this was not practical
until the `data.table` radix sorting was added to base R in version 3.3.0.

The concept is simple: order by group, compute cumulative sum, pull out the last
value for each group, and take the differences of these.  Visually:
...
x1 <- seq_len(n1)
y1 <- runif(n1);
g1 <- sample(c('#3333ee', '#33ee33', '#eeee33'), n1, replace=TRUE)
steps <- c(
  'Start', 'Sort By Group', 'Cumulative Sum',
  'Last Value in Group', 'Take Differnces', 'Group Sums!'
)
steps <- factor(steps, levels=steps)
df1 <- data.frame(x1, y1, g1, step=steps[[1]], stringsAsFactors=FALSE)
df2 <- df1[order(g1),]
df2[['x1']] <- x1
df2[['step']] <- steps[[2]]
df3 <- df2
...
df4 <- subset(
  ...
  c(head(g1, -1) != tail(g1, -1), TRUE)
)
df5 <- transform(df4, x1=3:5, y1=yc, step=steps[[5]])
df5 <- transform(df5, y1=c(y1[[1]], diff(y1)))
df6 <- transform(df5, step=steps[[6]])

ggplot(mapping=aes(x=x1, y=y1, fill=I(g1))) +
  geom_col(data=df1) +
  geom_col(data=df2) +
  geom_col(data=df3, position='waterfall') +
  geom_col(data=df4, mapping=aes(y=yc), width=0.9) +
...
1. Compute the cumulative sum ordered by groups.
2. Reset j
