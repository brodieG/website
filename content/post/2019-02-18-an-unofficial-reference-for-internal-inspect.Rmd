...
# Should We Care About Internal Details of R Objects?
...
because `tracemem` was quiet.  Let's try again, slightly differently:

```{r eval=FALSE}
identity(x)          # 'touch' again, with more emotion
```
```
 [1]  0  2  1  9  3  5  7 10  8  6
```
...
What we really need is a mechanism to understand why the first assignment
did not lead to a copy, but the second one did.  In both cases there
is only one reference to `x` so it should be safe to modify `x` without copy.

`.Internal(inspect(...))` allows us to inspect the guts of our R objects:
...
counting heuristic suggests there _could_ be more than one reference to the
object.
...
Closures always increment the `NAM` counter because R does not currently track
whether their environment is indeed destroyed on exit, so it assumes there could
possibly still be a reference to the parameter.  Primitives are
straight-to-compiled C code functions that have full control of memory and
reference count so they can leave it unchanged when warranted.
...
know about.  Only the cool kids on [r-devel][22] are clued in, and I only know
about it because my older cousin's college room mate's brother is one of the
cool kids.
...
object.  The 0 or 1 following the "g" corresponds to the value of the `gcgen`
bit of the meta data.  Additionally, R uses the [`mark` bit][16], and whether it
is set is shown by the "MARK" token appearing as highlighted here.  This allows
tracking of up to three generations of objects[^gen-count], where objects that
have a 1 value for the `mark` bit are considered older than those that don't.
...
[^symbol-hash]: This distinct from the string pool hash table.
