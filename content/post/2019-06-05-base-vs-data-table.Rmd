...
values, which we can do with[^na-caveat]:

```{r eval=FALSE}
go[-1L] != go[-length(go)]
```

So to get the unique values, including time to order:
...
Same result, but twice the speed of the original.  Most of the time is spent
ordering as we can see by how quickly we pick out the unique values once
the data is ordered:
...
real fight is up ahead.  And since we get to make the rules of engagement,
we're about to repeat the [Mayweather vs McGregor][106] fight... MMA style.

<span id=rowsums></span>Before we go on, I should note that R provides
`base::rowsums`, not to be confused with its better known cousin
...
regression, as we did in our [prior post][106]?  As a refresher this is what the
...
be slower as we would not be able to re-use the ordering data.
...
technicality.  In Part 3 of the Hydra Chronicles we will explore why we're
...
[106]: https://en.wikipedia.org/wiki/Floyd_Mayweather_Jr._vs._Conor_McGregor
