...
`base::rowsum` is a remarkable creature in the R ecosystem.  As far as I know,
it is the only base R function that computes group statistics on unequal group
sizes in statically compiled code.  Despite this it is arcane, especially when
compared with its popular cousin `base::rowSums`.
...
```
```{r rowsum-vs-rowsum, echo=FALSE, warning=FALSE}
p
...
```
```{r index-compare, echo=FALSE, fig.width=8.33, fig.height=5.21}
 ## Additional frames
...
p <- ggplot(id.dat, aes(x, y)) +
  ...
  coord_fixed(xlim=c(-2,23), ylim=c(-1,2.5)) +
  scale.fill + make.plot.theme
pdim <- gtable_dim(ggplotGrob(p))
old.opt <- knitr::opts_chunk$get(c('fig.width', 'fig.height'))
do.call(knitr::opts_chunk$set, list(fig.width=pdim[1], fig.height=pdim[2]))
```
```{r pad-index, echo=FALSE, warning=FALSE}
p
do.call(knitr::opts_chunk$set, old.opt)
```
...
p <- ggplot(id.dat.2, aes(x, y)) +
  ...
  geom_label(aes(label=glabel), size=2) +
  geom_text(data=el.ellipse, aes(label=lab), family='mono', vjust=0.5) +
  coord_fixed(xlim=c(-2,23), ylim=c(-3,2.5)) +
  scale.fill + make.plot.theme +
...
do.call(knitr::opts_chunk$set, list(fig.width=pdim[1], fig.height=pdim[2]))
```
```{r pad-index-diff, echo=FALSE, warning=FALSE}
p
do.call(knitr::opts_chunk$set, old.opt)
```
```{r echo=FALSE, eval=FALSE}
