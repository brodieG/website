...
dat20 <- rbind(
  dat10[order(g2)][, `:=`(label='main-back', alpha=faint, x=gx, y=gy)],
  dat10[order(g2)][, `:=`(x=gx, y=gy)]
)
dat20[, step:=2L]
...
dat40 <- rbind(
  fill=TRUE, group.longest, group.longest.end, dat30,
  data.table(
    id=seq_len(length(g.rle[['lengths']]) * g.max),
    x=rep(g.off, g.max * g.n), y=0,
    alpha=0, label='embed', type='tile'
  )
)
dat40[, step:=4L]

 # alloc matrix (move group labels and longest group)
...
dat50[
 ...
 `:=`(
    x=g.long.x,
    y=-c(13:12, c(12,3), c(3,2), c(2,0))
) ]
dat50[
  label == 'group-longest-end',
  `:=`(x=c(g.long.x), y=-c(g.max, 1) + 1)
]
dat50[label == 'group-lab', `:=`(x=x.off, y=0)]
dat50[label == 'embed', 
  `:=`(
    x=rep(x.off, g.max), 
    y=-rep(rev(seq_len(g.max)), each=g.n) + 1, alpha=1,
    tile.color='#AAAAAA'
  )
]
dat50[, step:=5L]
...
pads <- data.table(
  x=rep(g.off + (seq_len(g.n) - 1), 2),
  y=-c(rep(g.max, g.n), g.rle[['lengths']] + 1L) + 1L,
  id=rep(seq_len(g.n), 2)
)
pads.non.zero <- pads[, min(y), x][V1 > -g.max, x]
pads <- pads[x %in% pads.non.zero]

dat80 <- rbind(fill=TRUE,
  dat50[!label %in% c('group-longest', 'group-longest-end', 'group-lab')],
  copy(pads)[, `:=`(label='pad-line', type='path')],
  copy(pads)[, `:=`(label='pad-end', type='point')]
  # dat50[label == 'main'][, `:=`(
  #     x=g.n + g2 + 0,
...
dat90 <- rbind(fill=TRUE,
  ...
  dat80[label == 'main'][, `:=`(
      ...
      y=-sequence(g.rle[['lengths']]) + 1, g2=g.n+1L,
      label='embed-idx',
      type='text', glabel=as.character(id1)
) ] )
dat90[, step:=9L]
...
dat100 <- rbind(
  dat90[!label %in% c('main', 'pad-line', 'pad-end', 'embed-idx')],
  dat90[label == 'main'][, `:=`(
...
steps <- c(
  ...
  "Allocate Matrix with as Many Cols as Groups, and Rows as Longest Group Length",
...
dat <- rbind(
  dat10, dat20, dat30, dat40, dat50,
  dat80, dat90, dat100, dat110, dat120
)
...
make_plot <- function(dat, dat.code) {
  p <- ggplot(mapping=aes(x, y, alpha=alpha, group=paste(label, id))) +
    geom_tile(
      ...
      color=subset(dat, type=='tile')[['tile.color']]
    ) +
    ...
    geom_text(data=subset(dat, type=='text'), aes(label=glabel)) +
...
dat2 <- dat # dat[step %in% steps & type == 'tile']
dat2[, tile.color:=NA_character_]
p <- make_plot(dat2, NULL)
# p + facet_wrap(~step)
anim_save(
  '~/Downloads/colsums-anim.gif',
  p +  transition_states(step, wrap=FALSE) +
    labs(title = "{closest_state}"),
  width=800, height=557, nframes=length(unique(dat2$step)) * 2, fps=2
)
# p + facet_wrap(~step)
anim_save(
  '~/Downloads/colsums-anim.gif',
