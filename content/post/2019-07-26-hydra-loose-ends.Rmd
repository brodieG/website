...
The plotted times are based on running all steps (i.e. order, rle, sum)
together, and allocating the times across the three components on a best-efforts
basis.  The timings may not tie out exactly to the sub-component timings I show
...
```
  ...
  1.402   0.110   1.516 
```
This approach loses the key on cbind, so we either need a 
```
sys.time({
  DT <- data.table(grp, x, y)
  setkey(DT, grp)
...
```{r eval=FALSE}
g_slope_csum <- function(x, y, group) {
  o <- order(group)
  go <- group[o]
  xo <- x[o]
  yo <- y[o]
  ...
  x_ux <- xo - (xg/gn)[gi]
  y_ux <- yo - (yg/gn)[gi]
  og_sum_C(x_ux * y_ux, go)/og_sum_C(x_ux^2, go)
}
sys.time(gs.ns <- g_slope_Csum(x, y, grp))
```
```
   user  system elapsed 
  1.540   0.087   1.641 
