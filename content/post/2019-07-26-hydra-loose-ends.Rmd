...
group sizes in native code[^native-code].  Despite this it is relatively
...
```{r echo=FALSE}
step.levels <- c('order', 'compute', 'embed')
library(viridisLite)
funs <- c('tapply', 'tapply', 'rowsum', 'rowsum', 'data.table')
steps <- c('order', 'compute', 'order', 'compute', NA)
times <- data.frame(
...
ggplot(times, aes(x=Function, y=time)) +
  ...
  scale_fill_manual(values=viridis(3), na.value='grey50')
```

The slow step for `data.table` is likely the ordering too, but we don't have a
good way to break that out.

# colSums

`base::rowSums`, the aforementioned better-known cousin of `rowsum`, also
computes group statistics in native code.  Well, it kind of does if you consider
matrix rows to be groups. `base::colSums` does the same except for
columns.  Suppose we have three equal sized ordered groups:
...
head(cumsum(id0), 22)
```
A distinguishing feature of these manipulations, outside of being deathly
boring, is that they are all in fast vectorized code.  This gives us another
reasonably fast group sum function.  We split it up into a function that
...
This is a little slower than `rowsum` for the simple group sum, but as we'll see
later there are benefits to this approach.  The main drawback is the potential
for the embedding matrix to become inefficiently large if a small number of
groups are much larger than the rest.  It may be possible to mitigate this by
breaking up the data into sub by groups[^colsums-breakup].

# Pedal To The Metal

Out of curiosity I wrote a [C version of `rowsum`,
`og_sum_native`](#og_sum_native) that takes advantage of group ordered
...
reset the accumulators.  Even including sorting time, this ends up slightly
faster than `data.table`:

```{r}
sys.time(og_sum_native(xo, go))
```
```
   user  system elapsed 
  0.039   0.001   0.041 
...
Most notably the statistic computation takes virtually no time.  Let's compare
against all the different methods, including our prior [`cumsum` methods][110]:

ADD og_sum_native TIMINGS.

Benchmark plots needed:

* rowsum vs colsum, og_sum_native vs data.table
* slope w/ rowsum, colsum, og_slope_native vs data.table
...
ADD plot of all group sum timings, including the group slope ones.

Things get interesting when we start building more complex statistics.  As the
timings show a substantial portion of runtime is devoted to ordering and/or
rle/embedding computations.  These computations are a function of the grouping,
so in more complex statistics they can be re-used several times.

We're going to re-use the calculation of the bivariate regression slope:

$$\frac{\sum(x_i - \bar{x})(y_i - \bar{y})}{\sum(x_i -
\bar{x})^{2}}$$

The sorting time is now a longer as we need to sort `y` as well:
...
based group sum (`og_sum_col`).  This requires pre-sorted inputs, so we
will need to add the sorting time to it.  Don't worry too much about the
implementation.  The key thing to note is that we use `og_sum_col` four times,
but we only compute the embedding data once:

```{r}
og_slope_col <- function(xo, yo, go) {
...
sys.time(og_slope_col(xo, yo, go))
```
```
   user  system elapsed 
  1.362   0.709   2.078 
...
[`og_sum_native`](#pedal-to-the-metal) (the C version of `rowsum` that operates
...
```{r eval=FALSE}
og_slope_rsum <- function(xo, yo, go) {
...
sys.time(og_slope_rsum(xo, yo, go))
```
```
   user  system elapsed
  2.594   0.366   2.963
...
```{r eval=FALSE}
og_slope_nativesum <- function(xo, yo, go) {
  xg <- og_sum_native(xo, go)
  yg <- og_sum_native(yo, go)
  gn <- attr(xg, 'grp.size')
  gi <- rep(seq_along(gn), gn)

  x_ux <- xo - (xg/gn)[gi]
  y_ux <- yo - (yg/gn)[gi]
  og_sum_native(x_ux * y_ux, go)/og_sum_native(x_ux^2, go)
}
sys.time(gs.ns <- og_slope_nativesum(xo, yo, go))
```
```
   user  system elapsed
  0.463   0.086   0.550
```
```{r eval=FALSE}
sys.time(gs.n <- og_slope_native(xo, yo, go))
```
```
   user  system elapsed
  0.067   0.000   0.067
...
og_slope_native <- function(x, y, group) {
  stopifnot(
    typeof(x) == 'double', is.integer(group), length(x) == length(group),
    typeof(y) == 'double', length(x) == length(y)
  )
  tmp <- .og_slope_native(x, y, group)
  res <- setNames(tmp[[1]], tmp[[2]])
  res
}
.og_slope_native <- inline::cfunction(
...
[^native-code]: Code that is statically compiled into machine machine language
  and can be executed without any additional intervention from the R
  interpreter.
