...
ggplot(mapping=aes(x, y)) +
  ...
  geom_text(data=dat.rowsum, aes(label=val), size=lsize) +
  geom_text(data=dat.rowsum.res, aes(label=val), size=lsize) +
  geom_text(data=dat.rowSums, aes(label=val), size=lsize) +
  geom_text(data=dat.rowSums.res, aes(label=val), size=lsize) +
  geom_text(
...
tsize <- 5
dat.rowsum <- dat.rowsum[1:5]
dat.grp <- dat.rowsum[x == 1][, `:=`(x=x+2, facet='rowsum')]
dat.rowsum.res <- data.table(
  x=rep(1, each=3) + 20,
  y=rep(4:2, 1),
...
dat.lapply <- copy(dat.tapply.grp)[, `:=`(x=21, y=y+c(.5,.5,0,-.5, -.5))]
