...
```{r echo=FALSE}
 # Required sets:
 # 0: sortable/moveable colored squares
 # 1: alphaed background for colored squares
 # 2: group labels (for group count)
 # 3: longest group
 # 4: background for embedding matrix
...
faint <- 0.2
 ...
 # Main elements
dat0 <- data.table(
  step=1L, id=seq_along(g2), g2, x=gx, y=gx, alpha=1, label='main',
  glabel=NA_character_, type='tile'
)
 # Combine with background cells

dat10 <- rbind(
  copy(dat0)[, `:=`(label='main-back', alpha=faint)], dat0
)
 # Order foreground

dat20 <- rbind(
  dat10[label == 'main-back'],
...
dat30 <- rbind(
  ...
  dat20[label == 'main'][which(c(TRUE, diff(g2) != 0))][,
    `:=`(glabel=paste0('g', g2 - 1), label='group-lab', type='label')
  ]
)
dat30[, step:=3L]
...
g.off <- 11L
x.off <- seq_len(g.n - 1L) + 11
dat40 <- rbind(
  dat30[label != 'group-lab'],
  data.table(
    step=NA_integer_, id=seq_len(length(g.rle[['lengths']]) * g.max),
    g2=NA_integer_,
    x=rep(x.off, g.max), y=0
    alpha=1, label='embed', glabel=NA_character_
  )
...
group.longest <- data.table(
  id=rep(1:4, each=2),
  x=c(4, 5, 5, 5, 5, 6, 6, 6), y=-c(9, 9, 9, 0, 0, 0, 0, 2)
  alpha=1, label='group-longest', type='path'
)
group.longest.end <- data.table(
  x=c(4, 6), y=-c(9, 2), label='group-longest-end', id=5:6, type='point'
),

dat50 <- rbind(fill=TRUE, dat40, group.longest, group.longest.end)
dat50[, step:=5L]

 # longest-group moved

dat60 <- rbind(fill=TRUE,
  dat40,
  copy(group.longest)[,
    `:=`(
      x=x.off + which.max(g.rle[['lengths']]),
      y=-c(13:12, 12:2, 2:0)
    )
  ],
  copy(group.longest.end)[,
    `:=`(x=c(x.off + which.max(g.rle[['lengths']])), y=-c(13, 0))
  ]
)
dat60[, step:=6L]

 # rows to fit longest group

dat70 <- copy(dat60)
dat70[, step:=7L]
dat70[label=='embed', y:=-rep(rev(seq_len(g.max) - 1L), each=g.n)]

 # compute padding

pads <- data.table(
  x=rep(x.off + (seq_len(g.n) - 1), 2),
  y=-c(g.max, g.rle[['lengths']]) + 1L,
)
dat80 <- rbind(fill=TRUE,
  copy(dat70),
  copy(pads)[, `:=`(label='pad-line', type='path')],
  copy(pads)[, `:=`(label='pad-point', type='point')],
  copy(dat70)[
    label == 'main', `:=`(
      x=g.n + g2, y=-sequence(g.rle[['lengths']]) g2=10L, label='embed-main'
    )
  ]
)
dat80[, step:=8L]
...
dat90 <- rbind(fill=TRUE,
  dat80[!label %in% c('pad-line', 'pad-point')] ,
  dat80[
    label == 'embed-main',
    `:=`(type=='text', glabel=, label='embed-idx')



 # copy into embedding

dat80 <- dat70[!label %in% c('group-longest', 'group-longest-end')]
dat80[
  label == 'main', `:=`(
    x=g.n + g2, y=-sequence(g.rle[['lengths']]), step=8L
  )
]
 # colSums

dat90 <- copy(dat80)
dat90[label == 'main', y:=0]
dat90[, step:=9L]
...
steps <- c(
  ...
  "Order by Group",
  "Count Groups",
  "Allocate One Column Per Group",
  "Compute Length of Longest Group",
  "Allocate Longest Group Length as Rows",
  "Copy Each Group Into a Column",
  "Compute colSums"
)
dat <- transform(
...
make_plot <- function(dat, dat.code) {
  ggplot(aes(x, y, alpha=alpha, group=paste(label, id))) +
    geom_tile(data=subset(dat, type=='tile'), aes(fill=g2), color='#AAAAAA') +
    geom_point(data=subset(dat, type=='point')) +
    geom_path(data=subset(dat, type=='path')) +
    geom_label(data=subset(dat, type=='label'), aes(label=glabel)) +
    # we save the last group color for an off-scale color
    scale_fill_manual(
      values=c(viridisLite::viridis(length(unique(dat[['g2']]) - 1L)), 'grey50')
