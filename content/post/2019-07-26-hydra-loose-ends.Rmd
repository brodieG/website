...
sys.time({
  ...
  x_ux <- xo - ux[gi]
  y_uy <- yo - uy[gi]
  res <- rowsum(x_ux * y_uy, go) / rowsum(x_ux ^ 2, go)
})
sys.time({
  o <- order(grp)
  go <- grp[o]
  ...
  res <- ogroup_sum(x_ux * y_ux, go)/ogroup_sum(x_ux^2, go)
})
...
`data.table` and the "Gforce" optimized sum remain ahead:

```{r}

sys.time(setkey(data.table(x, grp), grp)[, sum(x), keyby=grp])
