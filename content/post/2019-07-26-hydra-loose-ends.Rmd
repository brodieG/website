...
steps <- c(
  ...
  "Allocate Embedding Marix",
  "Compute Padding in Embedding Matrix",
  "Compute Embedding indices",
  "Copy Each Group Into Its Column",
  "Compute colSums",
  # "End",
  NULL
)
dat <- rbind(
  dat10, dat20, dat30, dat40, dat50, dat60,
  dat70, dat80, dat90, dat100, dat110
)
...
make_plot <- function(dat, dat.code) {
  p <- ggplot(mapping=aes(x, y, alpha=alpha, group=paste(label, id))) +
    ...
    geom_point(
      data=subset(dat, type=='point'), size=3,
...
#   width=800, height=557, end_pause=5, nframes=50, fps=5
# )
# anim_save(
#   '~/Downloads/colsums-anim.gif',
#   p +  transition_states(Step, wrap=FALSE, state_length=1) +
#     labs(title = "{closest_state}"),
#   width=800, height=557, end_pause=2, nframes=350, fps=25
# )
#
## pngs for flipbook
...
i <- 11
png(sprintf(png.root, i + i.off), width=800, height=557)
dat.sub <- subset(dat, Step==steps[[i]])
dat.code.sub <- subset(dat.code, Step==steps[[i]])
p <- make_plot(dat.sub, NULL) + ggtitle(steps[[i]]) +
