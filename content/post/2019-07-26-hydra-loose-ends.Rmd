...
the group sum quickly, we use it to compute many other group statistics quickly
as well.  In this post we'll go over a few of the other group sum strategies I
tested out before I settled on the [`cumsum` based approach][110] we used to
"beat" `data.table`.
...
Despite `rowsum` and `rowSums` being near indistinguishable on name alone, they
do quite different things.  `rowsum` collapses rows together by group, leaving
column count unchanged, whereas `rowSums` collapses all columns in each row into
...
because the grouping is the same across all calculations.  This is particularly
obvious in the [`colSums` based group slope calculation in the
appendix](#g_sum_col).
...
Phew, finally done.  I never imagined this whole thing would get so out of hand
when I set off to do some silly benchmarks against `data.table`. I learned way
more than I bargained for, and come away from the whole thing with a renewed
admiration for what R does: it provides near-statically-compiled
performance[^near-statically] in an interpreted language right out of the box.
It's not always easy to achieve this, but in many cases common in data science
it is possible with a little thought and care.
...
* [Oleg Sklyar][123], [Dirk Eddelbuettel][122], [Romain François][121], etal. for
  [`inline`][] for easy integration of C code into ad-hoc R functions.
...
[^c-times]: Timing ordering and running `og_sum_C` separately versus together
  ...
  sorting / rle times.
[^near-static]: I consider ~3x close given that typically differences between
  statically compiled and interpreted coder are more on the order of ~10-100x.
