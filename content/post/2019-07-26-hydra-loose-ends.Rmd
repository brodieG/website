...
dat30 <- transform(
  dat20, step='longest',
  alpha=ifelse(g==g.rle[['values']][which.max(g.rle[['lengths']])], 1, .3)
)
g.max <- nrow(subset(dat30, alpha==1))
dat30 <- rbind(
  dat30,
  transform(subset(dat30, alpha==1), g=NA, alpha=0, label=1)
)
dat40 <- rbind(
...
dat40 <- do.call(
  ...
  c(
    list(dat40),
    replicate(
      gn - 1L, transform(subset(dat40, label==1), alpha=0), simplify=FALSE
) ) )
dat40 <- transform(dat40, step='nrow')
dat50 <- transform(dat40, step='allocate')
dat50[dat50[['label']] == 1,] <- transform(
  subset(dat50, label == 1),
  x=rep((seq_len(gn)) + gn, each=g.max), alpha=1
)
dat60 <- transform(
  rbind(
    subset(dat50, label==1),
    transform(
      subset(dat50, label==0), x=gn + 1 + g, y=sequence(g.rle[['lengths']])
    )
  ),
  step='embed'
)
dat <- transform(
  rbind(dat10, dat20, dat30, dat40, dat50, dat60),
  step=factor(step, levels=levels)
)

ggplot(dat) +
  geom_tile(aes(x, y, fill=g, alpha=alpha), color='#CCCCCC') +
  scale_fill_viridis_c(guide="legend", breaks=1:10) +
  facet_wrap(~step)
