...
In this post we'll go over a few of the other group sum strategies I tested out
before I settled on the [`cumsum` based approach][110] we used to "beat"
`data.table`.
...
```{r rowsum-vs-rowsum, echo=FALSE, fig.width=7, fig.height=2}
set.seed(1)
vals <- sample(1:10)
group <- c(1,1,2,3,3)
...
dat.rowSums <- copy(dat.rowsum)[, `:=`(facet='rowSums', x=x+2)]
...
text.tapply <- data.table(
  ...
  y=-8,
  hjust=c(1, .5, 0, .5, 1, 0),
  label=c('tapply(', ',', ', sum)', '->', 'lapply(list(', '), sum)')
)
...
ggplot(mapping=aes(x, y)) +
  geom_tile(data=dat.rowsum, color='#AAAAAA') +
  geom_tile(data=dat.grp, aes(fill=as.factor(group)), color='#AAAAAA') +
  geom_tile(data=dat.rowsum.res, aes(fill=as.factor(group)), color='#AAAAAA') +
  geom_tile(data=dat.rowSums, color='#AAAAAA') +
  geom_tile(data=dat.rowSums.res, color='#AAAAAA') +
  geom_label(data=dat.rowsum, aes(label=val), size=lsize, label.padding=lpad) +
  geom_label(data=dat.rowsum.res, aes(label=val), size=lsize, label.padding=lpad) +
  geom_label(data=dat.rowSums, aes(label=val), size=lsize, label.padding=lpad) +
  geom_label(
    data=dat.rowSums.res, aes(label=val), size=lsize, label.padding=lpad
  ) +
  geom_text(
  ...
  # geom_text(
  #   data=text.tapply, aes(label=label, hjust=hjust),
  #   size=tsize, family='mono'
  # ) +
  facet_wrap(~facet) +
  coord_fixed(xlim=c(-5, 10)) +
  # coord_fixed(xlim=c(-3.5, 21)) +
  scale_fill_manual(values=viridisLite::viridis(3), guide=FALSE) +
  make.plot.theme +
  NULL
```
...
p <- make_plot(dat.sub.ann, NULL, lab.size=3) +
  geom_text(data=dat.text, aes(label=glabel), size=3, color='grey50')
p
...
ggplot(id.dat, aes(x, y)) +
  ...
  coord_fixed(xlim=c(-3,22), ylim=c(-1,2.5)) +
...
ggplot(id.dat.2, aes(x, y)) +
  ...
  geom_label(data=diff.dat, aes(label=lab, color=lab>1), size=2) +
  geom_text(data=grp.lab, aes(label=lab), hjust=0, vjust=.75) +
  geom_text(data=el.lab.2, aes(label=lab), hjust=1, family='mono') +
  geom_label(aes(label=glabel), size=2) +
  coord_fixed(xlim=c(-3,22), ylim=c(-3,2.5)) +
  scale.fill + make.plot.theme +
...
The indices always increment by one, except for the embedding vector, where they
increment by `$1 + pad$` at each group transition, shown in red.  `$pad$` is how
much empty space there is between the end of the group and the end of the column
it is embedded in.  Conversely we can compute the embedding indices if we figure
out the size of the column padding, which will be a function of the group size
and the largest group size.  Thankfully the `rle` computes the lengths of
...
group sums and counts, similar to our [re-implementation of `unique` for sorted
data][160].  We can detect group transitions any time the group vector changes,
and use that to store group values/counts and reset the accumulators.  Once the
data is sorted, this takes virtually no time:
...
method, but should scale better if there are some groups that are much larger
than most.

`data.table` gets pretty close to the C implementation, but only in the
reformulated form which comes with challenging precision issues.
...
If there is one thing I'd want the casual reader to take away from this post
series is that R does something remarkable: it allows for 
near-statically-compiled performance in an interpreted language right out of the
box.  It's not always easy to achieve this, but in many cases common in data
science it is possible with a little thought and care.
