...
v <- cbind(x=list(0, .5, 1), y=as.list(c(0, height, 0) + v.off))
...
plot_bary <- function(p, v) {
  ...
  points(v[,'x'], v[,'y'], cex=unlist(bc) * 5, pch=19)
...
the triangle.  Here we do it with colors:

```{r echo=FALSE, fig.width=7, fig.height=2.5}
v2 <- cbind(v, r=c(1, 1, 0), g=c(0,1,1), b=c(1,0,1))
...
plot_bary2 <- function(p, v) {
  ...
  bcw <- lapply(
    seq_len(3),
    function(x)
      Reduce('+', matrix(Map('*', v[,c('r','g','b')], bc), nrow=nrow(v))[,x])
  )
  p.col <- rgb(bcw[1], bcw[2], bcw[3])
  points(p, col='white', bg=p.col, pch=22, cex=3)
  points(
    v[,'x'], v[,'y'], cex=unlist(bc) * 5, pch=19,
    col=rgb(v[,'r'], v[,'g'], v[,'b'])
  )
  text(v[,'x'], v[,'y'], labels=labeler(bc), pos=c(1,3,1), offset=1)
}
...
And taken to the natural extreme:
...
v2 <- cbind(v, r=c(1, 1, 0), g=c(0,1,1), b=c(1,0,1))
vp <- lapply(v2, '[', rep(1, nrow(points)))
dim(vp) <- dim(v2)
dimnames(vp) <- dimnames(v2)

bc <- bary_l(points, vp)
inbounds <- Reduce('&', lapply(bc, '>=', 0))

clrs.raw <- lapply(
  apply(vp[, c('r','g','b')], 2, Map, f="*", bc),
  Reduce, f="+"
)
par(bg='#EEEEEE')
par(xpd=TRUE)
...
plot.new()
clrs.in <- lapply(clrs.raw, '[', inbounds)
clrs <- do.call(rgb, lapply(clrs.in, pmin, 1))
points(points[inbounds,], pch=15, col=clrs)
polygon(v2[,c('x', 'y')])
points(
  v2[,c('x', 'y')], pch=21, cex=5, bg=rgb(v2[,'r'], v2[,'g'], v2[,'b']),
  col='white'
)
```

to interpolate values from triangle vertices into the triangles
...
bl <- bary_l(points.all[c('x', 'y')], l.dat.all[,c('x','y')])

inbounds <- Reduce('&', lapply(bl, '>=', 0))
blg <- lapply(bl, "^", 1/1.4)
clrs <- lapply(
  apply(l.dat.all[, c('r','g','b')], 2, Map, f="*", bl),
  Reduce, f="+"
)
...
poly.x[seq_len(nrow(meshdf)) + rep(seq_len(max(meshdf$id)) - 1, each=3)] <-
  meshdf$x
poly.y <- rep(NA_real_, nrow(meshdf) / 3 * 4)
poly.y[seq_len(nrow(meshdf)) + rep(seq_len(max(meshdf$id)) - 1, each=3)] <-
  meshdf$y
...
par(mgp=numeric(3))
plot.new()
rng.x <- range(c(poly.x, res$x), na.rm=TRUE)
rng.y <- range(c(poly.y, res$y), na.rm=TRUE)
scale <- function(x, rng) (x - rng[1]) / diff(rng)

point.x <- scale(res$x, rng.x)
point.y <- scale(res$y, rng.y)

poly.x <- scale(poly.x, rng.x)
poly.y <- scale(poly.y, rng.y)

points(point.x, point.y, col=as.character(res$color), pch=15)
