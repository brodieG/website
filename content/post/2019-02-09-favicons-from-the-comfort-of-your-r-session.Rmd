...
Argentini][2].  This requires installing [`imagemagick`][] via Brew or otherwise,
which felt like too much work to make a silly thumbnail.
...
```
These are all loaded as a single `magick` object:
```{r eval=FALSE}
imgs
```
```{r echo=FALSE}
...
This is not the first time that I've tried to solve an annoying system problem
only to realize R provides a great solution.  Batch processing text files is
another thing I've done, where at first I struggled with a bash/shell solution
before implementing an R solution in minutes[^shell-comfort].  I'm always amazed
at just how much you can do with R.
