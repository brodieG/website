...
The best grids for the R implementation will be `$2^k + 1, n * 2^k + 1$`, which
is actually a pretty good set as it allows for tilings of sub-maps at the most
approximate level we could want.  For example, if we have a base tile size of 65
x 65, we can assemble any tiling of them into rectangles and we'll be assured to
have self consistent extracted meshes, with the trade-off that the most
approximate mesh may be made of triangles no larger than 65 to the side.
Depending on dimensions it is possible that some parts of the map may form
larger triangles.
...
Still better than being stuck with square grids.  In fact, since it is unlikely
we'll ever want the largest tolerance approximation this beco
