...
> interesting, and how they work.  As such usage of `rlang` in this post will
> deviate from current recommended `rlang` practices[^tidy-eval-recommend].
...
Unfortunately the `rlang` team prioritized their vision for a new
meta-programming interface over harmonizing with existing conventions.  This
results in re-use of lexical roots to mean different things, and also new
symbology for existing concepts:

*  `rlang::quo` is used similarly to `base::bquote`, but is lexically closer to
  `base::quote`, and at same time gives no indication of it's special
...
[^mostly-reasonable]: I actually like the name "quosure", and I understand the
  ...
  tried to reflect some of his arguments in this footnote, but obviously my
  biases permeate.
