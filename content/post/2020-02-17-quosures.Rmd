...
by the way it carries around an environment to use as an Enclosure.  Mostly they
are used to implement Domain Specific languages such as regression model or plot
...
and evaluates the command therein.  We can recover the formula environment from
the call to the formula, which is fortunate as otherwise this trick could not
work.  Let's try it:
...
bquote(.(f1) / .(f2))
eval2(bquote(.(f1) / .(f2)))
eval2(bquote(.(f1) / .(f2) + a))
```

Ha, half-assed quosures.  These don't even support adding data to the
Environment Chain, or deity-forbid using a formula as a _formula_, but we
