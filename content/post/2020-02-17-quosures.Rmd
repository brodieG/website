...
terminology, even though the superbly arcane "quasi-quote" is strictly more
correct.
...
Evaluation Environments.  But what allows us to correctly resolve
`l.per.cubic.i` in the Calling Environment also forces the lookup of `mean` in
...
We explicitly bypass `mean_with_base`'s Evaluation Environment and in so doing
lose the shield against conflicting function definitions.  `rlang`'s version on
the other hand works fine.  Ultimately the issue is that in the command
generated by `mean_with_*`:
...
to the command quoting itself again!  It's like one of those awful trick candles
you just can't blow out:
...
`withbq`.  Let's look at a usage example before the implementation:

```{r withbq, echo=FALSE, eval=FALSE}
withbq <- function(x, env=parent.frame()) {
  caller <- parent.frame()
...
The odd-looking `bquote(.(bquote)(.(exp)))`[^odd-bquote] is used to generate a
`bquote` command to evaluate in the Calling Environment.  But this is an
implementation detail that the user of `withbq` need not worry about.
