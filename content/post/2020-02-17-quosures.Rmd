...
mean_with_rlang2 <- function(dat, cmd) {
  cmd <- quo(mean(!!enquo(cmd)))          # wrap it in `mean`
  eval_tidy(cmd, dat)
}
...
we use names mapped in environments other than the global environment.  Let's
...
local({
  ...
  list(
    #base=mean_with_base(mtcars, disp * l.per.cubic.i),
    #rlang=mean_with_rlang(mtcars, disp * l.per.cubic.i),
    rlang2=mean_with_rlang2(mtcars, disp * l.per.cubic.i)
  )
})
```
...
```{r}
q_with <- function(x, env=parent.frame()) {
...
Plus there are some remarkable benefits from the `with` approach:

* We can use good-old `base::eval`; this is a big deal, we avoid a huge amount
  of complexity.
...
later relies on custom print methods which will cause confusion when they are
...
partial-evaluation), and data masks.  First quasi-quotation, which we'll do with
`bq_with`.  Let's look at a usage example before the implementation:

```{r bq_with, echo=FALSE}
bq_with <- function(x, env=parent.frame()) {
  caller <- parent.frame()
...
q5 <- local({
  q <- bq_with(a + .(a))
  a <- 42   # assign to `a` AFTER bq_with!
  q
})
q5
eval(q5)
...
```{r bq_with, eval=FALSE}
```

The odd-looking `bquote(bquote(.(exp)))` is used to generate a `bquote` command
to evaluate in the Calling Environment.  But this is an implementation detail
that the user of `bq_with` need not worry about.
...
```{r}
arg_bq_with <- function(x, caller=sys.frame(sys.parent(2))) {
  force(caller)
  cmd <- eval(bquote(.(substitute)(.(substitute(x)))), parent.frame())
...
mean_with_bquo <- function(dat, cmd) {
  cmd <- bquote(mean(.(arg_bq_with(cmd))))
  cmd <- mask(cmd, dat)
  eval(cmd)
}
```
...
str(
  zz <- local({
    local({
      list(
        bquote2(.(parent.frame())),
        parent.frame(),
        eval(quote(parent.frame())),
        eval(quote(parent.frame(2))),
        eval(quote(parent.frame(3))),
        sys.frames()[sys.parents()]
      )
    }, a)
  }, b)
)
...
We use `bquote` to generate the `mean` call, but could also have used `bq_with`.
...
f1 <- function() {
  list(sys.frame(sys.parent(2)), parent.frame(2))
}


f0 <- function() {
  bquote(.(list(sys.frame(sys.parent()), parent.frame())))
}

# must be a function. `local` doesn't do it.
f1 <- function() {
  structure(
    list(
      sys.frame(sys.parent(2)), parent.frame(2),
      sys.frame(sys.parent()), parent.frame(),
      dat=list(fr=sys.frames(), par=sys.parents(), call=sys.calls()),
      extra=list(parent.frame(3), parent.frame(4))
    ),
    class='env_record'
  )
}
...
print.env_record <- function(x, ...) {
  env_capt <- function(x)
      sub(".*environment: 0x([0-9a-f]*).*", "\\1", capture.output(print(x)))
  frames <- sapply(x[[c('dat', 'fr')]], env_capt)
  calls <- sapply(
    x[[c('dat', 'call')]], function(x) {
      attributes(x) <- NULL   # dump srcrefs
  ...
  writeLines(
    paste(
      format(
        c(
          "sys.frame(sys.parent(2))", 
          "parent.frame(2)",
          "sys.frame(sys.parent())",
          "parent.frame()"
      ) ),
      sapply(x[1:4], env_capt) 
    )
  )
  writeLines("")
  writeLines(
...
This appears to show that `parent.frame` is just following the call stack,
whatever it may be.

```
sys.frame(sys.parent(2)) <environment: R_GlobalEnv>
parent.frame(2)          7fb38a676d38
sys.frame(sys.parent())  7fb38a676b78
parent.frame()           7fb38a676b78

0 | 7fb38a676b78 | f0()       
1 | 7fb38a676d38 | evalq(f1())
2 | 7fb38a676b78 | evalq(f1())
1 | 7fb38a6772e8 | f1()       
```
...
local({
  ...
  list(
    # base=mean_with_base(mtcars, disp * l.per.cubic.i),
    # rlang=mean_with_rlang(mtcars, disp * l.per.cubic.i),
    bq_with=mean_with_bquo(mtcars, disp * l.per.cubic.i)
  )
})
...
f0 <- function() {
  f1 <- function() {
    list(
      evalq(sys.frame(sys.parent(2))),
      evalq(parent.frame(2))
    )
  }
  f1()
}
...
f0 <- function() {
  f1 <- function(e) {
    ...
    list(
      ...
      sys.frames(),
      sys.parents(),
      evalq(list(sys.frames(), sys.parents(), sys.parent()))
    )
  }
  e <- environment()
  f1(e)
...
mean_with_bquo <- function(dat, cmd) {
  env <- parent.frame()
  cmd <- bq_with(.(substitute(cmd)), env)
  cmd <- bquote(mean(.(cmd)))
  cmd <- mask(cmd, dat)
  eval(cmd)
}
...
mean_with_bquo <- function(dat, cmd) {
  cmd <- mask(bquote(mean(.(bq_with(.(substitute(cmd)), env)))), dat)
  eval(cmd)
}
```
