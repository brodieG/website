...
```{r}
arg_q_with <- function(x, caller=parent.frame(2)) {
  exp.s <- eval(bquote(.(substitute)(.(substitute(x)))), parent.frame())
  force(caller)
  # Wrong environment if promise was forwarded?
  bquote(with(.(caller), .(exp.s)))
}
```
