...
> This post is not a tutorial for `rlang`, and it is not endorsed by the `rlang`
> authors.  It is about understanding what quosures are, what makes them
> interesting, and how they work in the context of `rlang`.  Quosures started
> off as a prominent part of `rlang`, but have more recently been relegated to
> something closer to "implementation detail" status.  They remain central to
> `rlang`, but are no longer emphasized in the documentation.  Since usage of
> `rlang` in this post is expressly for the purposes of understanding quosures,
> it deviates from current recommended `rlang` practices[^tidy-eval-recommend].
...
"Non-Standard Evaluation" (NSE), now is a good time to check out my previous
...
If you were early on the `rlang` bandwagon you might have seen the term
"quasi-quotation" bandied about.  It turns out partial evaluation **is**
quasi-quotation.  It's just that [quasi-quotation][4] was coined by the
...
or (partial) "evaluation" terminology, even though "quasi-quote" is the correct
"formal" term outside of R.
...
R expressions.  We may create them with `rlang::quo`:
...
While the parallels between the two implementations are clear when they are
juxtaposed, they are not obvious from the function names alone.  The `rlang`
...
* `rlang::quo` is used similarly to `base::bquote`, but is lexically closer to
  `base::quote`, and at same time gives no indication of its special
  environment capture abilities in its name.
* `rlang::enquo` is used similarly to `base::substitute`, but bears no relation
  ...
  to me given that the "en" gives no indication of how `enquo` is
  different from `quo` (my personal experience was that the lexical similarity
  between `quo` and `enquo` made it harder to figure out what was going on).
...
Partly as a result the initial confusion when `rlang` was first released the
authors have shifted to different terminology in its documentation and adjusted
the interface somewhat.  For example, the "curly-curly" operator was added
replace the `quo(!!enquo(arg))` pattern so we could have written instead[^v040]:
...
While it's great the `rlang` team has responded to feedback and tried to adapt
it would have been nice to see a re-convergence towards existing R terminology
rather than toward Standard Seventeen[^mask].
...
not uniquely mapped in the global environment.  Let's try to trigger failures by
...
affected in corner cases[^subtly-affected].  There is also the recovery
...
Which leads us to[^outside-bquote]:
...
and the code has not been used outside of this post[^tests].
...
dots (`...`) potentially resolving to multiple environments.  Situations with
extensive meta-programming involving `...` argument forwarding will benefit most
from `rlang`.
...
complexity trade off that come with `rlang`.
...
* For the illustrations:
    ...
    * Daniel Adler, Duncan Murdoch, etal. for [`rgl`][59] with which I mocked up
      the scenes.
    * Simon Urbanek for the [PNG package][61] which I
      used while post-processing many of the images in this post.
    * The yellow brick road scene is based on the official
      [`rlang` SVG logo][58], which is licensed [CC0][57].
* [Dirk Eddelbuettel][62] for copy edits.
...
[59]: https://cran.r-project.org/package=rgl
[61]: https://cran.r-project.org/web/packages/png/index.html
[62]: https://github.com/eddelbuettel
...
[^uncomfortable]: I don't think it is documented that the attributes of a call
...
[^subtly-affected]: [It is not possible][23] to reference a custom, non-quosure
  tilde defined as part of a quosure's Environment Chain.
[^mostly-reasonable]: I actually like the name "quosure", and I understand the
  ...
  It's easier to collaborate if we all share the same foundations, and it seems
...
[^mask]: This is not entirely fair: for example what used to be called
  ...
  examples.  On the other hand there is now the use of "defuse" and similar to
...
[^outside-bquote]: `bquote` uses `eval`, which sets two evaluation contexts in a
  way that causes `parent.frame()` to behave in unpredictable ways.  To avoid
  issues it is better to call `parent.frame()` [outside of `bquote`](#quote_w).
