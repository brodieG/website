...
expressions (or partially evaluate them, depending on your life outlook - I'm more
of a expression-half-evaluated guy).  Let's look first at a trivial example:
...
Additionally as you can see above, quosures record the environment the expression
would have been evaluated in, had it not been captured.  Unlike typical
unevaluated expressions, quosures do no resolve when they are `eval`ed:
...
mean_in_data_rl <- function(dat, expr) {
  expr <- enquo(expr)                # capture user expression
  expr <- quo(mean(!!expr))          # wrap it in `mean`
  eval_tidy(expr, dat)
}
```
```{r echo=FALSE}
...
```{r}
mean_in_data_rl2 <- function(dat, expr) {
  expr <- quo(mean({{expr}}))     # notice: no enquo
  eval_tidy(expr, dat)
}
```
```{r echo=FALSE}
...
mask <- function(expr, dat) {
  if(is.language(expr) && length(expr) > 1L) {
    ...
    if(expr[[1L]] == as.name('with') && is.environment(expr[[2L]])) {
      expr[[2L]] <- list2env(dat, parent=expr[[2L]])
    }
    # recurse on each sub-expression
    expr[2L:length(expr)] <- lapply(expr[2L:length(expr)], mask, dat)
  }
  expr
}
```
...
```{r eval=FALSE}
      expr[[2L]] <- list2env(mask, parent=expr[[2L]])
```
...
mean_in_data_w <- function(dat, expr) {
...
substitute_w <- function(x) {
  ...
  # Capture the user expression by running substitute in the caller frame
  # We inline the substitute function diretly into the expression to avoid
  # conflicts with a possible user-defined `substitute`

  expr <- substitute(x)                    # expr == x
  expr <- bquote(.(substitute)(.(expr)))    # expr == (<substitute>)(x)
  expr <- eval(expr, caller)                # expr == a + .(b)

  # Do the partial evaluation in the caller of the caller, see the
  # bquote_w breakdown above for details

  expr <- bquote(.(bquote)(.(expr)))        # expr == (<bquote>)(a + .(b))
  expr <- eval(expr, caller2)               # expr == a + 2
