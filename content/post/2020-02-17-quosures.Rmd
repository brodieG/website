...
that the user of `bquo_with` need not worry about.
...
```{r}
sub_quo_with <- function(x, env=parent.frame(), caller=parent.frame(2)) {
  force(caller)
...
mean_with_bquo <- function(dat, cmd) {
  env <- parent.frame()
  # cmd <- bquo_with(.(substitute(cmd)), env)
  # cmd <- bquote(mean(.(cmd)))
  # cmd <- mask(cmd, dat)
  cmd <- mask(bquote(mean(.(bquo_with(.(substitute(cmd)), env)))), dat)
  eval(cmd)
}
mean_with_bquo <- function(dat, cmd) {
  cmd <- sub_quo_with(cmd)
...
local({
  ...
  list(
    base=mean_with_base(mtcars, disp * l.per.cubic.i),
    rlang=mean_with_rlang(mtcars, disp * l.per.cubic.i),
    bquo_with=mean_with_bquo(mtcars, disp * l.per.cubic.i)
  )
})
