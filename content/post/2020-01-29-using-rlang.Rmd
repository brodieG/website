...
directly to `group_by` with `&#x7b;&#x7b;&#x7d;&#x7d` and letting it do all the fancy footwork.
...
a call, and we can't tunnel them with the (`&#x7b;&#x7b;`). I sense the
(`&#x7b&#x7b;&#x7b;`) in our near-future, along with chaos in the nascent
rlinguist vim community[^vim-folds].
...
Every R NSE implementation involves this step in some form.  The
also does this.  The novel concept `rlang` adds is that
freshly quoted expressions must immediately be "unquoted" with `!!` for single
variables or `!!!` for `...` variables so that they may be used.  A bold
strategy that has now been partially walked back with `&#x7b;&#x7b;`, at least for single
variables.  More on this in a bit.
...
From this (and several other design decisions) I get the sense that building
`rlang` in a way that is complementary to precedent in R proper ranked low
on the list of priorities.  It would have been reasonably easy to come up with
complimentary names, or failing that more orthogonal ones.  Instead we have a
new NSE syntax that is hostile to the precedent.  I do believe this was not the
intent, but I find it





.  There is also `base::expression`, and you'd be forgiven to think
it might be similar to but the latter is actually the closest
thing to `bquote` in `rlang`.  `rlang::exprs` is probably the closest analogue
to `expression`.

I understand the appeal of tabula-rasa.  I don't expect that conforming to
existing standards that are flawed should overrule everything.  But I do believe
that if the tidy philosophy prioritized complementing rather than replacing base
R when it does not **substantively** compromise the mission the tidyverse would
be less divisive than it is today.
...
made extending and complementing existing semantics
an important part of design decisions frictions in the R community would be
lessened.
...
seemingly below closely hewing to principles of obscure, AFAICT mostly dead, CS
theory on fexprs.


was considered overwhelmingly more important.  Obviously I don't know
what Lionel
and Hadley were thinking when they put this together, but this model of
priorities is what I can make consistent with the observed results.  Either
aligning to the existing semantics or creating a distinct lexicon would have
been better, and neither would have been harder.  Instead the translation of
experience in base NSE to `rlang` and vice versa is perversely hindered by the
mixed semantics[^not-end-of-world].
...
Thing is R an incredibly flexible language.  You can make it
do just about anything you want, ill-advised or not.  The only substantive limit
is the parser.  Yet this is something that the parser had to be
here we have something that cannot be interpreted correctly
by the parser being .


something that I could
only bring myself to do if I gave exactly zero fucks about

another result that I
can only explain



The absurdity of this pattern led to the



I've been
able to think of some cases where this is usefu
can think
of some cases

Jkk
need to be separately marked for evaluation in order to evaluate.
This is a new feature, and it is directly

as in base you just evaluate the expression.  This
feature is directly responsible for



followed by the immediate unquote, i.e. they why are we doing this



I can only attribute to ideological purity is the
need to
...
Sorry, couldn't keep the lid completely on it.  At least the new `&#x7b;&#x7b;&#x7d;&#x7d`
moustache operator makes takes some of the edge off.





it is harder for experienced base NSE programmers to understand
`rlang`, and where `rlinguists` will similarly struggle with base NSE.


experienced base R NSE programmers trying to use `rlang` are
befuddled, and likely vice-versa

<div style='background-color: red;'>check old conversions on SO</div>



Maybe the original names weren't great, but using names that are
lexically close, semantically different, and in the same subfield interferes
with the existing language constructs.

This makes It appear as if minimal
consideration is given to future `rlinguists` who may have to work with base
NSE, or vice versa.  If this paragraph sounds a little faint and distorted it's
because I'm screaming it from that hill over yonder, the one I'm still waiting
to die on[^conf-update].


One of my beefs[^beef] with `rlang` is that it prioritizes **internal**
consistency and adherence to obscure CS principles at the expense of precedent
in R.  For example `base::substitute` does roughly what `enquo` does, but
`base::enquote` does something completely different.  `base::bquote` does
roughly what `expr` does, but `base::expression` does something different yet
again.



kk
And herein lies my major philosophical beef with `rlang` and to a lesser extent
the `tidyverse` in general .
...
And this brings me to my major philosophical beef with `rlang` and the
`tidyverse`



We could
try to avoid going this way

One
solution


We've had to cram a reasonably complex



I like that `rlang` added the double-mo-perator
...
e distinction is made because
  R also has functions that are not closures

The special thing about closures
  is that they have their own lexically scoped environments
