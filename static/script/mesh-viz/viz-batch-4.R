...
water_surface(f1, 50)
water_surface(f2, 50, t=pi/4)
water_surface(f3, 50, t=pi/2)
...
dy2 <- .4
dy3 <- .525
objrr3 <- obj_model(f3, material=mat.w, scale=rep(1.5/(2*pi), 3), y=dy3)
objrr2 <- obj_model(f2, material=mat.w, scale=rep(1.5/(2*pi), 3), y=dy2)
objrr1 <- obj_model(f1, material=mat.w, scale=rep(1.5/(2*pi), 3), y=dy1)
...
render_scenes(
  ...
  # scn.4b,
  #filename=file,
  filename='~/Downloads/mesh-viz/small-mesh/water-new7c-%d.png',
  height=rez, width=rez, samples=samp,
  # lookfrom=c(0, 3, 1.5), lookat=c(0, 0, 0),
  lookfrom=c(0, 3, 1.5), 
  lookat=c(0, .5, 0), fov=35,
